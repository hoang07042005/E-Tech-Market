...
- **Dừng hệ thống và XÓA SẠCH dữ liệu**: Tắt container và hủy bỏ toàn bộ dữ liệu database, cache lưu trong volumes. Dùng khi muốn reset môi trường Docker hoàn toàn sạch sẽ từ đầu.
  ...
  ```
- **Nạp (Import) dữ liệu mẫu vào DB Docker thủ công**: Nếu bạn không dùng file `.bat` tự động, bạn có thể nạp file `database_real_data_backup_utf8.sql` trực tiếp thông qua lệnh sau (đảm bảo container đang chạy `up -d`):
  ```bash
  # Dành cho Mac/Linux/Git Bash (Windows)
  docker-compose exec -T db psql -U postgres -d etech < database_real_data_backup_utf8.sql
  
  # Dành cho Windows Command Prompt (CMD) / PowerShell
  type database_real_data_backup_utf8.sql | docker-compose exec -T db psql -U postgres -d etech
  ```

### 2. Xem Trạng Thái & Log Hệ Thống
