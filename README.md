...
| **Actor** | **Use Cases** |
| :--- | :--- |
| **Khách hàng (Guest)** | Xem trang chủ • Danh sách SP theo danh mục • Lọc/Tìm kiếm • Xem chi tiết (PDP): gallery, variant picker, thông số, đánh giá, Shop Q&A • So sánh tối đa 3 sản phẩm • Giỏ hàng: thêm/cập nhật/xóa |
| **Khách hàng (Auth)** | Thanh toán: tạo đơn + tạo link MoMo/VNPAY + nhận callback • Quản lý đơn hàng: xem list/detail, hủy đơn, xác nhận đã nhận/đã thanh toán • Yêu cầu hoàn trả: request return + confirm refund • Wishlist: toggle yêu thích • Đánh giá sản phẩm • Thông báo: xem list + đánh dấu đã đọc • Blog & Video: xem list/detail + comment • Liên hệ hỗ trợ: gửi message |
| **Admin** | Dashboard stats • Quản lý đơn hàng: update trạng thái + duyệt/từ chối return request + mark refunded • Quản lý sản phẩm: CRUD + restock theo variant • Quản lý danh mục (categories) • Quản lý shipping zones/methods • Quản lý coupons • Quản lý flash sales: CRUD + add/remove items + bulk discount • Quản lý nội dung: blog posts, product news, videos, banners • Kiểm duyệt reviews, Shop Q&A (pending) • Xử lý contact messages, newsletter subscriptions |

### 📱 App (Mobile – Flutter)

| **Actor** | **Use Cases** |
| :--- | :--- |
| **Khách hàng (Guest)** | Home: banner slider, tab sản phẩm, flash sale, coupon section, newsletter • Browse sản phẩm + filter + load more • PDP + variant picker + flash sale price + thông số + liên quan |
| **Khách hàng (Auth)** | Shop Q&A: xem/đặt câu hỏi • Reviews: gửi đánh giá đa chiều • Wishlist: toggle & đồng bộ • Cart: thêm/cập nhật/xóa item • Checkout: áp coupon + chọn MoMo/VNPAY/COD + xem kết quả • Orders: list/detail + cancel + confirm nhận hàng + confirm thanh toán • Return request: request return + confirm refund • Notifications: xem + mark read • Account: cập nhật profile/avatar, đổi mật khẩu, xem voucher/coupon & purchased products • Blog/Video: xem list/detail • Maintenance mode: hiển thị màn hình bảo trì |
| **Admin** | Dashboard stats • Quản lý sản phẩm • Quản lý tồn kho (restock variant) • Quản lý đơn hàng |

### ⚙️ Backend (Laravel API)

| **Phân Khúc** | **Use Cases** |
| :--- | :--- |
| **Public (Không Auth)** | Auth: register/login/google login/forgot password/reset password • Verify email • Health check (`/health`) • Categories, Products (list/detail/related) • Flash sale current • Banners, Videos • Reviews list • Blog: categories/posts/show + comment • Newsletter subscribe • Store config/contact/payments/shipping • Shop Q&A (list/store) • Payment callbacks: VNPAY return/ipn, MoMo return/ipn |
| **Auth (Khách hàng)** | User profile: get/patch, avatar, change password, sessions • Cart: show/add/update/remove items • Orders: create (từ cart hoặc từ items), index/show • Orders lifecycle: cancel, confirm-received, confirm-payment • Return & refund: request return + confirm refund received • Payments create link: MoMo/VNPAY theo từng order • Wishlist: index/toggle • Notifications: index/mark read • Reviews: store review cho sản phẩm • Coupons: list coupons, apply coupon, lưu voucher/coupon cá nhân (saved) |
| **Admin** | Dashboard stats • Orders management + return request approve/reject/refunded • Products management + restock variant • Categories CRUD • Shipping zones/methods CRUD • Roles/users CRUD (RBAC) • Reviews moderation + destroy • Contact messages moderation • Coupons CRUD • Flash sales CRUD + manage items/bulk discount • Product news CRUD (per product) • Blog posts CRUD + blog categories • Upload thumbnails • Banners CRUD • Videos CRUD |
