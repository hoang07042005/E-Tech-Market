...
graph LR
    subgraph ActorsW
        GW["Khách hàng<br/>(Guest)"]
        AU["Khách hàng<br/>(Authenticated)"]
        AD["Admin"]
    end

    GW --> UC1["Xem trang chủ"]
    GW --> UC2["Danh sách SP<br/>(category)"]
    GW --> UC3["Lọc/Tìm kiếm"]
    GW --> UC4["PDP<br/>(gallery, variant, specs, reviews, Q&A)"]
    GW --> UC5["So sánh SP"]
    GW --> UC6["Giỏ hàng"]

    AU --> UC7["Thanh toán<br/>(MoMo/VNPAY)"]
    AU --> UC8["Quản lý đơn"]
    AU --> UC9["Hoàn trả"]
    AU --> UC10["Wishlist"]
    AU --> UC11["Đánh giá"]
...
graph LR
    subgraph ActorsA
        GA["Khách hàng<br/>(Guest)"]
        LA["Khách hàng<br/>(Logged in)"]
        FA["Admin"]
    end

    GA --> U15["Home"]
    GA --> U16["Browse + Filter"]
    GA --> U17["PDP"]
...
graph LR
    subgraph ActorsB
        CL["Client<br/>(Web/App)"]
        US["Khách hàng<br/>(Auth)"]
        SA["Admin"]
    end

    subgraph Public
        P1["Auth<br/>(login/register/forgot)"]
        P2["Verify email"]
        P3["Health check"]
        P4["Public Data<br/>(categories, products, banners, videos, reviews, blog, newsletter, store, Q&A)"]
        P5["Payment<br/>(callbacks)"]
    end
...
| 🌐 Web Client | 📱 App Client | ⚙️ Backend Public | 🔐 Backend Auth | 🔧 Backend Admin |
| :--- | :--- | :--- | :--- | :--- |
| Xem trang chủ | Home | Auth | Profile | Dashboard |
| Danh sách SP | Browse + Filter | Verify email | Cart | Orders Mgmt |
| Lọc/Tìm kiếm | PDP | Health check | Orders | Products |
| PDP (gallery, variant, specs, reviews, Q&A) | Shop Q&A | Public Data | Return & Refund | Categories |
| So sánh SP | Reviews | Payment callbacks | Payments | Shipping |
| Giỏ hàng | Wishlist | — | Wishlist | RBAC |
| Thanh toán | Cart | — | Notifications | Reviews Mod |
| Quản lý đơn | Checkout | — | Reviews | Messages |
| Hoàn trả | Orders | — | Coupons | Coupons |
| Wishlist | Return | — | — | Flash Sales |
| Đánh giá | Notifications | — | — | News |
| Thông báo | Account | — | — | Blog |
| Blog & Video | Blog/Video | — | — | Banners |
| Liên hệ | Maintenance | — | — | Videos |
