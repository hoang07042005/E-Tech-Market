...
class OrdersController extends Controller
{
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        return DB::transaction(function () use ($user, $cart, $items, $data) {
            $subtotal = $items->reduce(fn($carry, $it) => $carry + ($it->product->price * $it->quantity), 0);
            ...
            foreach ($items as $it) {
                ...
                if ($activeFlashSale) {
                    ...
                    $saving = ((float)$it->product->price - (float)$activeFlashSale->flash_sale_price) * $it->quantity;
            ...
            foreach ($items as $it) {
                ...
                OrderItem::query()->create([
                    ...
                    'total_price' => $product->price * $it->quantity,
                ]);
            }
        ...
        return DB::transaction(function () use ($user, $data, $itemsInput, $paymentMethod) {
            ...
            foreach ($itemsInput as $it) {
                ...
                if ($variantId) {
                    ...
                    if ($variant) $originalPrice = (float) $variant->price;
                }
            ...
            foreach ($itemsInput as $it) {
                ...
                if ($activeFlashSale) {
                    ...
                    $originalPrice = (float) $product->price;
                    if (isset($it['variant_id'])) {
                        $variant = $product->variants()->where('id', $it['variant_id'])->first();
                        if ($variant) $originalPrice = (float) $variant->price;
                    }
            ...
            foreach ($itemsInput as $it) {
                ...
                if ($variantId) {
                    $variant = $product->variants()->where('id', $variantId)->first();
                    if ($variant) $unitPrice = (float) $variant->price;
                }
