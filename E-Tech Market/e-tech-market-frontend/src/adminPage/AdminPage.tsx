    ...
    <div className={`adminLayout ${isSidebarCollapsed ? 'collapsed' : ''}`}>
      ...
      <aside className="adminSidebarPremium">
        ...
        <nav className="adminSidebarNav">
          ...
          <SidebarItem active={activeTab === 'products'} onClick={() => setActiveTab('products')} icon={<BoxIcon />} label="Sản phẩm" collapsed={isSidebarCollapsed} />
          <SidebarItem active={activeTab === 'flashSale'} onClick={() => setActiveTab('flashSale')} icon={<FlashIcon />} label="Flash Sale" collapsed={isSidebarCollapsed} />
          <SidebarItem active={activeTab === 'orders'} onClick={() => setActiveTab('orders')} icon={<CartIcon />} label="Đơn hàng" collapsed={isSidebarCollapsed} />

          <SidebarSection title="QUẢN LÝ" collapsed={isSidebarCollapsed} />
          <SidebarItem active={activeTab === 'productNews'} onClick={() => setActiveTab('productNews')} icon={<NewspaperIcon />} label="Tin sản phẩm" collapsed={isSidebarCollapsed} />
          <SidebarItem active={activeTab === 'categories'} onClick={() => setActiveTab('categories')} icon={<GridIcon />} label="Danh mục" collapsed={isSidebarCollapsed} />
          <SidebarItem active={activeTab === 'shopQna'} onClick={() => setActiveTab('shopQna')} icon={<ChatBubbleIcon />} label="Hỏi đáp cửa hàng" collapsed={isSidebarCollapsed} />
          <SidebarItem active={activeTab === 'reviews'} onClick={() => setActiveTab('reviews')} icon={<StarIcon />} label="Đánh giá" collapsed={isSidebarCollapsed} />
          <SidebarItem active={activeTab === 'contactMessages'} onClick={() => setActiveTab('contactMessages')} icon={<MailIcon />} label="Liên hệ" collapsed={isSidebarCollapsed} />
          <SidebarItem active={activeTab === 'coupons'} onClick={() => setActiveTab('coupons')} icon={<TicketIcon />} label="Mã giảm giá" collapsed={isSidebarCollapsed} />
          <SidebarItem active={activeTab === 'blog'} onClick={() => setActiveTab('blog')} icon={<NewspaperIcon />} label="Tin tức Blog" collapsed={isSidebarCollapsed} />
          <SidebarItem active={activeTab === 'notifications'} onClick={() => setActiveTab('notifications')} icon={<BellIcon />} label="Thông báo" collapsed={isSidebarCollapsed} />
          <SidebarItem active={activeTab === 'users'} onClick={() => setActiveTab('users')} icon={<UserGroupIcon />} label="Người dùng" collapsed={isSidebarCollapsed} />

          <SidebarSection title="CÀI ĐẶT" collapsed={isSidebarCollapsed} />
            ...
            <div className="adminNotificationWrap" ref={notifWrapRef}>
              ...
              {notifOpen ? (
                <div className="adminNotifMenu" role="menu">
                  ...
                  {notifLoading ? (
                    <div className="adminNotifEmpty">Đang tải…</div>
                  ) : !notifRows.length ? (
    ...
    async function load() {
      ...
      try {
        const [res, products] = await Promise.all([
          apiFetch<DashStats>(`/api/admin/dashboard/stats?range=${encodeURIComponent(analyticsRange)}`, { token }),
          ...
          <div className="admAnalyticsGrid">
            <div className="admAnalyticsChart">
              {(() => {
                ...
                const coords = pts.map((p, i) => ({ x: toX(i), y: toY(p.value), label: p.label, value: p.value }))

                // Monotone cubic spline (Fritsch–Carlson) → smooth line without ugly kinks/overshoot
                const smoothLine = (cs: Array<{ x: number; y: number }>) => {
                  ...
                  // Hyman filter to avoid sharp changes
                  for (let i = 0; i < n - 1; i++) {
                    if (m[i] === 0) {
                      t[i] = 0
                      t[i + 1] = 0
                  ...
                  for (let i = 0; i < n - 1; i++) {
                    ...
                    const cp1y = y0 + (t[i] * hSeg) / 3
                    const cp2x = x1 - hSeg / 3
                    const cp2y = y1 - (t[i + 1] * hSeg) / 3
                    d += ` C ${cp1x.toFixed(1)} ${clamp(cp1y, padY, h - padY).toFixed(1)} ${cp2x.toFixed(1)} ${clamp(cp2y, padY, h - padY).toFixed(1)} ${x1.toFixed(1)} ${y1.toFixed(1)}`
                  }
                  return d
                }

                const dLine = smoothLine(coords)
                const baseY = h - padY
                const dArea = `${dLine} L ${coords[coords.length - 1].x.toFixed(1)} ${baseY.toFixed(1)} L ${coords[0].x.toFixed(1)} ${baseY.toFixed(1)} Z`

                const ticks = 4
                const tickVals = Array.from({ length: ticks + 1 }, (_, i) => min + (max - min) * (i / ticks))
                const yLabel = (v: number) => `${fmtMoneyShort(v)} đ`
                const maxIdx = coords.reduce((best, c, i) => (c.value > coords[best].value ? i : best), 0)
                const lastNonZeroIdx = (() => {
                  for (let i = coords.length - 1; i >= 0; i--) if (coords[i].value > 0) return i
                  return coords.length - 1
                })()

                return (
                  <>
                    <svg className="admAreaSvg" viewBox={`0 0 ${w} ${h}`} preserveAspectRatio="xMidYMid meet" aria-hidden>
                      <defs>
                        <linearGradient id="admAreaFill" x1="0" x2="0" y1="0" y2="1">
                          <stop offset="0%" stopColor="#fb923c" stopOpacity="0.14" />
                          <stop offset="100%" stopColor="#fb923c" stopOpacity="0.01" />
                        </linearGradient>
                        <linearGradient id="admLineStroke" x1="0" x2="1" y1="0" y2="0">
                          <stop offset="0%" stopColor="#fb923c" />
                          <stop offset="60%" stopColor="#f97316" />
                          <stop offset="100%" stopColor="#ea580c" />
                        </linearGradient>
                        <filter id="admLineShadow" x="-20%" y="-20%" width="140%" height="140%">
                          <feDropShadow dx="0" dy="2" stdDeviation="2.2" floodColor="#fdba74" floodOpacity="0.16" />
                        </filter>
                      </defs>
                      {tickVals.map((tv, i) => {
                        const y = toY(tv)
                        return (
                          <g key={i}>
                            <line className="admAreaGridLine" x1={padL} x2={w - padR} y1={y} y2={y} />
                            <text className="admAreaYAxisText" x={padL - 10} y={y + 4} textAnchor="end">
                              {yLabel(tv)}
                            </text>
                          </g>
                        )
                      })}
                      <path d={dArea} fill="url(#admAreaFill)" />
                      <path d={dLine} fill="none" stroke="url(#admLineStroke)" strokeWidth="2.4" strokeLinejoin="round" strokeLinecap="round" filter="url(#admLineShadow)" opacity="0.92" />
                      {coords.map((c, idx) => (
                        <g key={idx}>
                          <circle cx={c.x} cy={c.y} r={(idx === maxIdx || idx === lastNonZeroIdx) ? 4.5 : 3.2} fill="#fff" stroke="#9a5a12" strokeWidth="2" />
                          {(idx === maxIdx || idx === lastNonZeroIdx) && c.value > 0 && (
                            <text className="admAreaValueText" x={c.x} y={c.y - 12} textAnchor="middle">
                              {fmtMoneyShort(c.value)} đ
                            </text>
                          )}
                        </g>
                      ))}
                    </svg>
                    <div className="admAreaLabels" aria-hidden>
                      {coords.map((c, idx) => (
                        <span key={idx} className="admAreaLabel">{c.label}</span>
                      ))}
                    </div>
                  </>
                )
          ...
          <div className="admTableWrap">
            <table className="admTable">
              ...
              <tbody>
                {dashLoading ? (
                  <tr>
                    <td colSpan={8} style={{ padding: 16, color: '#64748b' }}>Đang tải…</td>
                  </tr>
...
function MedalIcon() {
  return (
    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden>
      ...
      <path d="M8 3h4l-2 3H6l2-3zm8 0h-4l2 3h4l-2-3z" />
    </svg>
  )
}
