...
export default function OrdersAdminPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      setError(null)
      try {
        const data = await apiFetch<OrdersResponse>(`/api/admin/orders?${queryString}`, { token })
        if (cancelled) return
        setRes(data)
      } catch (e) {
        if (cancelled) return
  ...
  useEffect(() => {
    ...
    async function loadDetail() {
      ...
      setDetailError(null)
      try {
        const d = await apiFetch<OrderDetail>(`/api/admin/orders/${selectedId}`, { token })
        if (cancelled) return
        setDetail(d)
  ...
  const saveDetail = async () => {
    ...
    setDetailError(null)
    try {
      const updated = await apiFetch<OrderDetail>(`/api/admin/orders/${detail.id}`, {
        token,
        method: 'PATCH',
  ...
  const approveReturnRequest = async () => {
    ...
    setRrError(null)
    try {
      const updated = await apiFetch<OrderDetail>(`/api/admin/orders/${detail.id}/return-request/approve`, {
        token,
        method: 'POST',
        body: JSON.stringify({ admin_note: rrNote.trim() ? rrNote.trim() : null }),
      })
      setDetail(updated)
  ...
  const rejectReturnRequest = async () => {
    ...
    setRrError(null)
    try {
      const updated = await apiFetch<OrderDetail>(`/api/admin/orders/${detail.id}/return-request/reject`, {
        token,
        method: 'POST',
        body: JSON.stringify({ admin_note: rrNote.trim() }),
      })
      setDetail(updated)
      updateRowReturnRequest(updated)
    } catch (e) {
  ...
  const markRefunded = async () => {
    ...
    try {
      ...
      const updated = await apiFetch<OrderDetail>(`/api/admin/orders/${detail.id}/return-request/refunded`, {
        token,
        method: 'POST',
        body: fd,
      })
