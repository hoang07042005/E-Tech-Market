...
import '../css_admin/UsersAdminPage.css'
...
export default function UsersAdminPage() {
  ...
  useEffect(() => {
    ...
    async function fetchRolesCatalog() {
      ...
      try {
        const list = await apiFetch<Role[]>('/api/admin/roles', { token })
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const res = await apiFetch<PaginatedUsers>(`/api/admin/users?${q.toString()}`, { token })
      setRows(res.data ?? [])
  ...
  return (
    <div className="prodAdminRoot">
      ...
      <div className="prodTableWrap">
        {loading ? (
          <div className="prodLoading">Đang tải danh sách…</div>
        ) : (
