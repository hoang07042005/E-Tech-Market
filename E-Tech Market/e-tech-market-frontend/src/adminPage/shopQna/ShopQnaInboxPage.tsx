...
import '../css_admin/ShopQnaInboxPage.css'
...
export default function ShopQnaInboxPage() {
  ...
  async function load() {
    ...
    setError(null)
    try {
      const rows = await apiFetch<PendingShopQna[]>('/api/admin/shop-qna/pending', { token })
      setItems(rows)
    } catch (e: unknown) {
  ...
  return (
    <div className="sqInboxRoot">
      ...
      {loading ? (
        <div className="sqInboxLoading">Đang tải…</div>
      ) : items.length === 0 ? (
