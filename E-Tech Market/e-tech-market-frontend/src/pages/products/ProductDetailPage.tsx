...
import '../css_pages/ProductDetailPage.css'
...
function PdpThumbStrip({ allImages, selectedImg, onSelectImage }: PdpThumbStripProps) {
  ...
  const showThumbNav = thumbTotal > PDP_THUMB_VISIBLE
  const thumbMaxStart = Math.max(0, thumbTotal - PDP_THUMB_VISIBLE)
  const thumbStart = Math.min(thumbWindowStart, thumbMaxStart)
  const thumbsToRender = showThumbNav
    ? allImages.slice(thumbStart, thumbStart + PDP_THUMB_VISIBLE)
    : allImages
  ...
  const pick = (img: string) => {
    ...
    setThumbWindowStart(s => {
      ...
      if (idx >= effective + PDP_THUMB_VISIBLE) return idx - PDP_THUMB_VISIBLE + 1
      return effective
    })
  }
        ...
        <div className="pdpFacetSection">
          ...
          <div className="pdpFacetChipRow pdpFacetChipRow--storages" role="list">
            {visibleStorages.map(st => {
              ...
              const priceStr = vOpt
                ? `${parseFloat(vOpt.price).toLocaleString('vi-VN')} đ`
                : '—'
              return (
                <button
                  key={st}
...
export default function ProductDetailPage() {
  ...
  if (loading) return <div className="pdpLoading">Loading excellence...</div>
        ...
        <div className="pdpMainGrid">
          ...
          <div className="pdpInfo">
            ...
            <div className="pdpPriceRow">
              ...
              {(activeFlashSale || (product.old_price && !selectedVariant)) && (
                <span className="pdpOldPrice">
                  {activeFlashSale 
                    ? `${parseFloat(selectedVariant ? selectedVariant.price : product.price).toLocaleString('vi-VN')} đ`
                    : parseFloat(product.old_price!).toLocaleString('vi-VN') + ' đ'
                  }
                </span>
              )}
            </div>
                        ...
                        return (
                          ...
                          >
                            ...
                            <span className="pdpVariantFallbackChip__price">
                              {parseFloat(v.price).toLocaleString('vi-VN')} đ
                            </span>
                          </button>
                        )
                      })}
            ...
            <div className="pdpActions">
              ...
              <button
                ...
                onClick={() => {
                  ...
                  let price = 0
                  if (activeFlashSale) {
                    price = Number.parseFloat(activeFlashSale.flash_sale_price.toString())
                  } else {
                    const priceStr = v ? v.price : product.price
              ...
              {relatedProducts.map(rp => (
                <div key={rp.id} className="pdpRelatedCard">
                  <Link to={`/products/${rp.slug}`} className="pdpRelatedTop">
                    {rp.old_price && (
                      <span className="pdpRelBadge pdpRelBadge--discount">
                        {(() => {
                          const p = parseFloat(rp.price)
                          const op = parseFloat(rp.old_price)
                          if (!isFinite(p) || !isFinite(op) || op <= 0 || op <= p) return null
                  ...
                  <div className="pdpRelatedInfo">
                    ...
                    {rp.old_price && (
                      <div className="pdpRelatedOldRow">
                        <span className="pdpRelatedOldLabel">Giá hãng mới:</span>{' '}
                        <span className="pdpRelatedOldPrice">{parseFloat(rp.old_price).toLocaleString('vi-VN')}đ</span>
                      </div>
                    )}

                    {rp.old_price && (
                      <div className="pdpRelatedSaveRow">
                        {(() => {
                          const p = parseFloat(rp.price)
                          const op = parseFloat(rp.old_price || '0')
                          if (!isFinite(p) || !isFinite(op) || op <= p) return null
                          const save = op - p
                          return <span className="pdpRelatedSaveTag">Tiết kiệm: {save.toLocaleString('vi-VN')}đ</span>
                        })()}
                      </div>
                    )}
