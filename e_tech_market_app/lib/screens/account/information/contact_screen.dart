...
class _ContactScreenState extends State<ContactScreen> {
  ...
  String get _displayName =>
      (_storeContact?['store_name']?.toString().trim().isNotEmpty == true)
          ? _storeContact!['store_name']
          : _fallbackStoreName;
  String get _displayPhone =>
      (_storeContact?['contact_phone']?.toString().trim().isNotEmpty == true)
          ? _storeContact!['contact_phone']
          : _fallbackPhone;
  String get _displayEmail =>
      (_storeContact?['contact_email']?.toString().trim().isNotEmpty == true)
          ? _storeContact!['contact_email']
          : _fallbackEmail;
  String get _displayAddress =>
      (_storeContact?['warehouse_address']?.toString().trim().isNotEmpty ==
              true)
    ...
    try {
      ...
      if (res.statusCode == 201 || res.statusCode == 200) {
        ...
        AppSnackBar.showSuccess(
            context, 'Đã gửi liên hệ! Chúng tôi sẽ phản hồi bạn sớm.');
      }
      ...
      if (data is Map && data['message'] != null) {
        ...
        AppSnackBar.showError(
            context, 'Gửi liên hệ thất bại. Vui lòng thử lại.');
      }
    } catch (e) {
      if (mounted)
        AppSnackBar.showError(context, 'Lỗi kết nối. Vui lòng thử lại.');
    } finally {
                    ...
                    : Column(
                        ...
                        children: [
                          ...
                          _buildInfoRow(
                              ...
                              'Email hỗ trợ',
                              _displayEmail,
                              () => _launchUrl('mailto:$_displayEmail')),
                          const SizedBox(height: 14),
                          _buildInfoRow(
              ...
              child: Container(
                ...
                child: Form(
                  ...
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      const Text('Gửi tin nhắn cho chúng tôi',
                          style: TextStyle(
                              fontWeight: FontWeight.bold, fontSize: 16)),
                      ...
                      _buildField('Họ và tên', 'Nguyễn Văn A', _nameCtrl,
                          validator: (v) => v!.trim().length < 2
                              ? 'Tên tối thiểu 2 ký tự'
                              : null),
                      const SizedBox(height: 14),
                      _buildField(
                          'Địa chỉ Email', 'example@email.com', _emailCtrl,
                          ...
                          validator: (v) =>
                              !RegExp(r'^[^@]+@[^@]+\.[^@]+').hasMatch(v ?? '')
                                  ...
                                  : null),
                      const SizedBox(height: 14),
                      _buildField('Số điện thoại', '0901 234 567', _phoneCtrl,
                      ...
                      TextFormField(
                        ...
                        validator: (v) => v!.trim().length < 5
                            ? 'Vui lòng nhập nội dung (tối thiểu 5 ký tự)'
                            : null,
                      ),
                      const SizedBox(height: 20),
