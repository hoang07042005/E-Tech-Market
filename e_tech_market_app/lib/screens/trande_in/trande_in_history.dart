...
class _TradeInHistoryScreenState extends State<TradeInHistoryScreen> {
  ...
  final List<Map<String, dynamic>> _filters = [
    {'key': 'all', 'label': 'Tất cả yêu cầu', 'icon': Icons.grid_view_rounded, 'color': const Color(0xFFFF5722)},
    {'key': 'completed', 'label': 'Đã thu mua', 'icon': Icons.check_circle_outline, 'color': Colors.green},
    {'key': 'processing', 'label': 'Đang xử lý', 'icon': Icons.hourglass_empty, 'color': const Color(0xFFEF7A45)},
    {'key': 'pending', 'label': 'Chờ kiểm tra', 'icon': Icons.search, 'color': Colors.blue},
    {'key': 'approved', 'label': 'Đã hoàn tất', 'icon': Icons.assignment_turned_in_outlined, 'color': Colors.purple},
    {'key': 'rejected', 'label': 'Đã hủy', 'icon': Icons.cancel_outlined, 'color': Colors.grey.shade600},
  ];

  static const Map<String, Map<String, String>> _statusMap = {
    'pending': {'label': 'Chờ kiểm tra', 'filterKey': 'pending', 'color': 'gray'},
    'quoted': {'label': 'Đang xử lý', 'filterKey': 'processing', 'color': 'orange'},
    'approved': {'label': 'Đã hoàn tất', 'filterKey': 'approved', 'color': 'green'},
    'rejected': {'label': 'Đã hủy', 'filterKey': 'rejected', 'color': 'red'},
    'completed': {'label': 'Đã thu mua', 'filterKey': 'completed', 'color': 'green'},
  };
  ...
  Color _getStatusColor(String colorKey) {
    switch (colorKey) {
      ...
      case 'orange': return const Color(0xFFEF7A45);
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text('Lịch sử Thu cũ', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 18, color: Theme.of(context).colorScheme.onSurface)),
        backgroundColor: Theme.of(context).colorScheme.surface,
        elevation: 0,
        centerTitle: false,
        ...
        children: [
          ...
          Container(
            ...
            decoration: BoxDecoration(
              ...
              border: Border(bottom: BorderSide(color: Theme.of(context).colorScheme.onSurface.withOpacity(0.1), width: 1)),
            ),
                  ...
                  child: Container(
                    ...
                    child: Text(
                      ...
                      style: TextStyle(
                        ...
                        color: isSelected ? const Color(0xFFFF5722) : const Color(0xFF8B95A5),
                      ),
                    ),
                  ),
                );
                ...
                : ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      final req = filteredRequests[index];
                          ...
                          final thumb = imgs.isNotEmpty ? NetworkUtils.fixDeviceUrl(imgs[0]) : null;
                          
                          final info = _parseInfoLines(req['machine_info']);
                          final name = info['name'] as String;
                          final specs = info['specs'] as List<Map<String, String>>;
                          final shortSpec = specs.take(4).map((s) => s['val']).join(" • ");
                          
                          final createdAt = DateTime.tryParse(req['created_at']?.toString() ?? '');
                          final dateStr = createdAt != null ? '${createdAt.day.toString().padLeft(2, '0')}/${createdAt.month.toString().padLeft(2, '0')}/${createdAt.year}' : '';
                          ...
                          final conditionStr = conditions != null && conditions.isNotEmpty
                              ? conditions.map((c) {
                                  final name = c['name'] as String;
                                  final parts = name.split(':');
                                  return parts.length > 1 ? parts.sublist(1).join(':').trim() : name.trim();
                                }).join(', ')
                              : '';
                              
                          final updatedAt = DateTime.tryParse(req['updated_at']?.toString() ?? '');
                          ...
                          final hasAnyPrice = estPrice != null || finalPrice != null;
                                      ...
                                      children: [
                                        Row(
                                          children: [
                                            ...
                                            Icon(Icons.access_time, size: 14, color: Theme.of(context).colorScheme.onSurface),
                                            const SizedBox(width: 4),
                                            Text(
                                              timeStr,
                                              style: TextStyle(fontSize: 12, color: Theme.of(context).colorScheme.onSurface, fontWeight: FontWeight.w500),
                                        ...
                                        Container(
                                          ...
                                          decoration: BoxDecoration(
                                            color: _getStatusColor(statusInfo['color']!).withOpacity(0.1),
                                            borderRadius: BorderRadius.circular(6),
                                            border: Border.all(color: _getStatusColor(statusInfo['color']!).withOpacity(0.3)),
                                          ),
                                          child: Row(
                                            mainAxisSize: MainAxisSize.min,
                                            children: [
                                              Icon(Icons.check_circle_outline, size: 12, color: _getStatusColor(statusInfo['color']!)),
                                              const SizedBox(width: 4),
                                              Text(
                                                statusInfo['label']!,
                                                style: TextStyle(
                                                  color: _getStatusColor(statusInfo['color']!),
                                                  fontSize: 12,
                                      ...
                                      children: [
                                        ...
                                        Expanded(
                                          child: Column(
                                            ...
                                            children: [
                                              Text(
                                                name.isNotEmpty ? name : (req['category']?['name'] ?? 'Thiết bị'),
                                    ...
                                    if (conditions != null && conditions.isNotEmpty) ...[
                                      ...
                                      Wrap(
                                        ...
                                        children: conditions.map<Widget>((c) {
                                          ...
                                          final title = parts.length > 1 ? parts.sublist(1).join(':').trim() : cName.trim();
                                          return Container(
                                            ...
                                            child: Row(
                                              mainAxisSize: MainAxisSize.min,
                                              children: [
                                                const Icon(Icons.error_outline, size: 15, color: Color(0xFFEA580C)),
                                      ...
                                      children: [
                                        Expanded(
                                          child: Column(
                                            crossAxisAlignment: CrossAxisAlignment.start,
                                            children: [
                                              Text('Giá dự kiến', style: TextStyle(fontSize: 12, color: Theme.of(context).colorScheme.onSurface)),
                                              const SizedBox(height: 4),
                                              Text(
                                                estPrice != null ? '${_formatPrice(estPrice)} đ' : 'Chưa có',
                                                style: const TextStyle(fontSize: 15, fontWeight: FontWeight.bold, color: Color(0xFFFF5722)),
                                              ),
                                            ],
                                          ),
                                        ),
                                        ...
                                        Expanded(
                                          child: Column(
                                            ...
                                            children: [
                                              Text(
                                                finalPrice != null ? 'Giá thu mua' : 'Cập nhật lần cuối', 
                                              ...
                                              Text(
                                                finalPrice != null 
                                                  ...
                                                  : (updatedDateStr.isNotEmpty ? updatedDateStr : 'Chưa có'),
                                                style: TextStyle(
                                                  fontSize: finalPrice != null ? 15 : 13, 
                                                  fontWeight: finalPrice != null ? FontWeight.bold : FontWeight.normal, 
                                                  color: finalPrice != null ? Colors.green.shade600 : Theme.of(context).colorScheme.onSurface
                                                ),
                                              ),
                                            ],
                                          ),
