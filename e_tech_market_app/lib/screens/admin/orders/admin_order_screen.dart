  ...
  Future<void> _loadOrders() async {
    ...
    try {
      final data = await AdminOrdersService.fetchAdminOrders(
        page: _currentPage,
        orderCode: _orderCodeCtrl.text.trim().isNotEmpty
            ? _orderCodeCtrl.text.trim()
            : null,
        customer: _customerCtrl.text.trim().isNotEmpty
            ? _customerCtrl.text.trim()
            : null,
        dateFrom: _dateFromCtrl.text.trim().isNotEmpty
            ? _dateFromCtrl.text.trim()
            : null,
        dateTo:
            _dateToCtrl.text.trim().isNotEmpty ? _dateToCtrl.text.trim() : null,
        status: _filterStatus != 'all' ? _filterStatus : null,
        paymentMethod:
            _filterPaymentMethod != 'all' ? _filterPaymentMethod : null,
        paymentStatus:
            _filterPaymentStatus != 'all' ? _filterPaymentStatus : null,
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        centerTitle: true,
        title: Text(Trans.orderManagementAdmin,
          ...
          child: Column(
            ...
            children: [
              ...
              if (_isLoading)
                ...
                Center(
                    child: Text('Đã xảy ra lỗi: $_error',
                    ...
                    if (pagination.isNotEmpty && pagination['last_page'] > 1)
                      Row(
                        ...
                        children: [
                          ...
                          Text(
                              'Trang $_currentPage / ${pagination['last_page']}',
                              style: const TextStyle(
                                  fontWeight: FontWeight.bold, fontSize: 13)),
                          IconButton(
                            icon: const Icon(Icons.arrow_forward_ios_rounded,
                                size: 14),
                            onPressed: _currentPage < pagination['last_page']
                                ? () {
                                    setState(() => _currentPage++);
                                    _loadOrders();
                                  }
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Text('Khách hàng: ${row['customer_name'] ?? '—'}',
              style: TextStyle(
                  color: Theme.of(context).colorScheme.onSurface,
                  fontSize: 13)),
          const SizedBox(height: 4),
          Text('Ngày đặt: ${_formatDisplayDate(row)}',
              style: TextStyle(
                  color: Theme.of(context).colorScheme.onSurface,
                  fontSize: 13)),
          ...
          Text('Phương thức thanh toán: ${row['payment_method'] ?? '—'}',
              style: TextStyle(
                  color: Theme.of(context).colorScheme.onSurface,
                  fontSize: 13)),
          const SizedBox(height: 4),
                ...
                child: ElevatedButton(
                  onPressed: () {
                    Navigator.push(
                      ...
                      MaterialPageRoute(
                          builder: (context) =>
                              AdminOrderDetailScreen(orderId: row['id'])),
                    ).then((_) => _loadOrders());
                  },
                  style: ElevatedButton.styleFrom(
                    backgroundColor: const Color(0xFF4F46E5),
