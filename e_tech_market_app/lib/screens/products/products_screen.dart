...
class _ProductsScreenState extends State<ProductsScreen> {
  ...
  Future<void> _fetchWishlist() async {
    ...
    if (mounted) {
      setState(() {
        _wishSet = items.map<int>((i) => (i['product_id'] as num).toInt()).toSet();
      });
    }
  }
  ...
  Future<void> _toggleWishlist(int productId) async {
    ...
    if (status == null && mounted) {
      ...
      setState(() {
        if (status == 'added') _wishSet.add(productId);
  ...
  Future<void> _fetchProducts() async {
    ...
    String? sortParams;
    String? orderParams;
    if (_sortValue == 'Mới nhất') { sortParams = 'created_at'; orderParams = 'desc'; }
    else if (_sortValue == 'Cũ nhất') { sortParams = 'created_at'; orderParams = 'asc'; }
    else if (_sortValue == 'Giá: Thấp -> Cao') { sortParams = 'price'; orderParams = 'asc'; }
    else if (_sortValue == 'Giá: Cao -> Thấp') { sortParams = 'price'; orderParams = 'desc'; }
                        ...
                        builder: (BuildContext context, StateSetter setModalState) {
                          return ListView(
                            ...
                            children: [
                              ...
                              ..._categories.map((c) => _buildCategoryNode(c, 0, setModalState)),
                              ...
                              Row(
                                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                                children: [
                                  const Text('MỨC GIÁ', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14)),
                                  Row(
                                    children: [
                                      const Text('Nhập khoảng giá', style: TextStyle(fontSize: 12)),
                                      Checkbox(
                              ...
                              if (!_useCustomPrice) ...[
                                SliderTheme(
                                  data: SliderTheme.of(context).copyWith(
                                    ...
                                    overlayColor: const Color(0xFFF26522).withOpacity(0.2),
                                    trackHeight: 4.0,
                                  ),
                                  child: Slider(
                                    min: 0,
                                    max: _absoluteMaxPrice,
                                    value: _maxPrice.clamp(0.0, _absoluteMaxPrice),
                                    onChangeEnd: (val) => _fetchProducts(),
                                    onChanged: (val) {
                                      setModalState(() { _maxPrice = val; _priceTouched = true; });
                                      setState(() { _maxPrice = val; _priceTouched = true; });
                                    },
                                  ),
                                ),
                                Row(
                                  mainAxisAlignment: MainAxisAlignment.spaceBetween,
                                  children: [
                                    const Text('0 đ', style: TextStyle(color: Colors.grey, fontSize: 12)),
                                    Text('Tối đa ${_formatPrice(_maxPrice)} đ', style: TextStyle(color: Colors.grey, fontSize: 12)),
                                  ],
                                ),
                              ] else ...[
                                Row(
                                  children: [
                                    Expanded(
                                      child: TextField(
                                        ...
                                        onChanged: (val) {
                                          ...
                                          setModalState(() { _minPrice = v; _priceTouched = true; });
                                          setState(() { _minPrice = v; _priceTouched = true; });
                                          
                                          if (_debounce?.isActive ?? false) _debounce!.cancel();
                                          _debounce = Timer(const Duration(milliseconds: 1000), () => _fetchProducts());
                                        },
                                      ),
                                    ),
                                    ...
                                    Expanded(
                                      child: TextField(
                                        ...
                                        onChanged: (val) {
                                          double v = double.tryParse(val) ?? 0;
                                          setModalState(() { _maxPrice = v; _priceTouched = true; });
                                          setState(() { _maxPrice = v; _priceTouched = true; });
                              ...
                              const SizedBox(height: 24),
                              // HÃNG
                              const Text('HÃNG', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14)),
                              const SizedBox(height: 12),
                              _brands.isEmpty 
                              ? const Text('Chưa có dữ liệu hãng.', style: TextStyle(color: Colors.grey, fontSize: 12))
                              : Wrap(
                                spacing: 8,
                                runSpacing: 8,
                                children: _brands.map((b) => InkWell(
                                  onTap: () {
                                    setModalState(() {
                                      _selectedBrand = _selectedBrand == b ? null : b;
                                    });
                                    setState(() {
                                      _selectedBrand = _selectedBrand == b ? null : b;
                                    });
                                  ...
                                  child: Container(
                                    width: (MediaQuery.of(context).size.width * 0.85 - 40) / 2, // 2 columns
                                    padding: const EdgeInsets.symmetric(vertical: 10),
                                    decoration: BoxDecoration(
                                      border: Border.all(
                                        color: _selectedBrand == b ? const Color(0xFFF26522) : Colors.grey.shade300,
                                        width: 1,
                                      ),
                                    ...
                                    child: Text(
                                      ...
                                      style: TextStyle(
                                        ...
                                        color: _selectedBrand == b ? const Color(0xFFF26522) : Colors.black87,
                                      ),
                                    ),
                                  ),
                                )).toList(),
        ...
        child: Row(
          children: [
            Icon(
              ...
              color: isSelected ? const Color(0xFFF26522) : Colors.grey.shade400,
              size: 20,
            ),
            const SizedBox(width: 12),
            Expanded(
  ...
  Widget _buildCategoryNode(Map<String, dynamic> c, int level, StateSetter setModalState) {
    final hasChildren = c['children'] != null && (c['children'] as List).isNotEmpty;
      ...
      children: [
        InkWell(
          onTap: () {
            if (hasChildren) {
              setModalState(() {
                if (isExpanded) _expandedCategories.remove(c['id']);
                else _expandedCategories.add(c['id']);
              });
              setState(() {});
            } else {
              setModalState(() => _selectedCategoryId = isSelected ? null : c['id']);
          ...
          child: Padding(
            ...
            child: Row(
              children: [
                ...
                if (hasChildren)
                  Icon(isExpanded ? Icons.arrow_drop_up : Icons.arrow_drop_down, color: Colors.black54),
              ],
            ),
          ),
        ),
        if (hasChildren && isExpanded)
          ...(c['children'] as List).map((child) => _buildCategoryNode(child, level + 1, setModalState)),
      ],
    );
  }
  ...
  double _getDisplayPrice(Map<String, dynamic> product) {
    ...
    if (variants != null && variants.isNotEmpty) {
      ...
      sortedVariants.sort((a, b) => ((a['effective_price'] ?? 0) as num).compareTo((b['effective_price'] ?? 0) as num));
      return (sortedVariants[0]['effective_price'] as num).toDouble();
    }
  ...
  double? _getDisplayOldPrice(Map<String, dynamic> product) {
    ...
    if (variants != null && variants.isNotEmpty) {
      final sortedVariants = List.from(variants);
      sortedVariants.sort((a, b) => ((a['effective_price'] ?? 0) as num).compareTo((b['effective_price'] ?? 0) as num));
      final originalPrice = double.tryParse(sortedVariants[0]['price'].toString()) ?? 0.0;
      final finalPrice = (sortedVariants[0]['effective_price'] as num).toDouble();
  ...
  Widget _buildProductCard(Map<String, dynamic> product) {
    ...
    String imageUrl = product['main_image_url']?.toString() ?? '';
    if (imageUrl.isNotEmpty) {
      imageUrl = fixDeviceUrl(imageUrl);
    }
              ...
              ClipRRect(
                ...
                child: Container(
                  ...
                  child: imageUrl.isEmpty 
                    ? Center(child: Icon(Icons.computer, size: 80, color: Colors.grey.shade300))
                    : Image.network(
                        imageUrl,
                        width: double.infinity,
                        height: 220,
              ...
              children: [
                Row(
                  ...
                  children: [
                    Text(
                      ...
                      style: TextStyle(color: Colors.grey.shade600, fontSize: 12, fontWeight: FontWeight.bold, letterSpacing: 1),
                    ),
                    Row(
                      children: List.generate(5, (index) {
                        if (index < rating.floor()) {
                          ...
                          return Icon(Icons.star, size: 14, color: Colors.grey.shade300);
                        }
                      }),
                    ),
                  ],
                ...
                Row(
                  ...
                  children: [
                    Text(
                      ...
                      style: const TextStyle(color: Color(0xFFF26522), fontSize: 18, fontWeight: FontWeight.bold),
                    ),
                ...
                SizedBox(
                  ...
                  child: ElevatedButton(
                    ...
                    child: const Text('THÊM VÀO GIỎ', style: TextStyle(fontWeight: FontWeight.bold, letterSpacing: 0.5)),
                  ),
                ),
              ],
            ),
  ...
  Widget build(BuildContext context) {
    return ListView(
      children: [
        // Header Texts
        Text(
          _selectedCategoryId != null ? 'SẢN PHẨM THEO DANH MỤC' : 'TẤT CẢ SẢN PHẨM',
          style: const TextStyle(
            fontSize: 20,
            fontWeight: FontWeight.bold,
            letterSpacing: 1.5,
          ...
          children: [
            ...
            SizedBox(
              ...
              child: OutlinedButton.icon(
                ...
                label: const Text('BỘ LỌC', style: TextStyle(color: Colors.black87, fontWeight: FontWeight.bold, fontSize: 13)),
        ...
        if (_isLoading)
          ...
          const Padding(
            padding: EdgeInsets.all(40.0),
            child: Center(child: Text('Không có sản phẩm phù hợp bộ lọc của bạn.')),
          )
        else
          ListView.builder(
            shrinkWrap: true,
