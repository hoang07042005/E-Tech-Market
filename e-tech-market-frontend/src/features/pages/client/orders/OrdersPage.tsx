...
type OrderListRow = {
  ...
  items?: Array<
    | { product_name_snapshot?: string | null; main_image_url?: string | null; quantity?: number }
    | { product?: { name?: string | null; main_image_url?: string | null } | null; quantity?: number }
    | null
  >
}
...
function getItemQty(it: NonNullable<NonNullable<OrderListRow['items']>[number]>): number {
...
export default function OrdersPage() {
  ...
  useEffect(() => {
    if (!hasAuth) { navigate('/login'); return }
    let cancelled = false
    setTimeout(() => { if (!cancelled) { setLoading(true); setError(null) } }, 0)
    apiFetch<OrdersIndexResponse>(`/orders?page=${page}`)
      .then(d  => { if (!cancelled) setRes(d) })
      .catch(e => { if (!cancelled) setError(e instanceof Error ? e.message : 'Không tải được đơn hàng.') })
  ...
  const filteredRows = useMemo(() => {
    ...
    let result = rows.filter(r => {
      if (q.trim()) {
        const hay = ((r.order_code || '') + ' ' + getItemNames(r.items).join(' ')).toLowerCase()
      ...
      if (timeRange !== 'all') {
        if (timeRange === 'custom') {
          if (customFrom && customTo && r.created_at) {
            ...
            if (!Number.isFinite(t) || t < Date.parse(customFrom) || t > Date.parse(customTo)) return false
          }
        } else {
          ...
          if (Number.isFinite(days) && r.created_at) {
            ...
            if (current != null && (current - t) / (1000 * 60 * 60 * 24) > days) return false
          }
        }
      }
      return true
  ...
  const kpis = useMemo(() => {
    ...
    const shipping  = rows.filter(r => (r.status || '').toLowerCase() === 'shipped').length
    const completed = rows.filter(r => ['completed', 'delivered'].includes((r.status || '').toLowerCase())).length
  ...
  // Skeleton
  const SkeletonRow = () => (
    <div className="odRow" style={{ display: 'flex', gap: 16, padding: '16px 20px', alignItems: 'center' }}>
  ...
  const content = loading ? (
    ...
    <div className="od-empty-state">
      <div className="od-empty-icon"><BoxIcon /></div>
      <h3 className="od-empty-title">Bạn chưa có đơn hàng nào</h3>
      <p className="od-empty-desc">Hãy khám phá các sản phẩm tuyệt vời của E-Tech ngay hôm nay!</p>
      <Link to="/products" className="od-empty-btn">Mua sắm ngay</Link>
    </div>
  ) : filteredRows.length === 0 ? (
    <div className="od-empty-state">
      <div className="od-empty-icon"><SearchIcon /></div>
      <h3 className="od-empty-title">Không tìm thấy đơn hàng</h3>
      <p className="od-empty-desc">Thử thay đổi bộ lọc hoặc từ khóa tìm kiếm.</p>
    </div>
  ) : (
    <div className="odhList">
      {filteredRows.map(o => {
        ...
        return (
          <div key={o.id} className="odRow">
            ...
            <div className="odRowMid">
              ...
              <div className={`odRowProducts ${itemCount > 1 ? 'odRowProducts--multi' : ''}`}>
                {/* Thumbnails — up to 3 */}
                {imgs.length > 0 && (
                  <div className="odThumbStrip">
                    {imgs.slice(0, 3).map((src, i) => (
                      <img key={i} className="odThumb" src={src} alt="" loading="lazy" />
                    ))}
                  </div>
                )}
                {imgs.length === 0 && (
                ...
                <div className="odNameList">
                  ...
                  {names.length > 3 && (
                    <div className="odNameMore">+ {names.length - 3} sản phẩm khác</div>
                  )}
                </div>
              </div>
            </div>
            ...
            <div className="odRowRight">
              ...
              <div className="odRowActions">
                ...
                <button type="button" className="odMenuBtn" aria-label="Tùy chọn">
                  <DotsIcon />
                </button>
              </div>
            </div>
  ...
  const pageNumbers = useMemo(() => {
    ...
    if (last <= 7) {
      ...
      for (let i = Math.max(2, cur - 1); i <= Math.min(last - 1, cur + 1); i++) nums.push(i)
    ...
    <>
      ...
      {!loading && !error && rows.length > 0 && (
        <div className="odhKpis">
          <div className="odhKpi">
            <div className="odhKpiIcon odhKpiIcon-orange"><BoxIcon /></div>
          ...
          <div className="odhKpi">
            <div className="odhKpiIcon odhKpiIcon-yellow"><ClockIcon /></div>
            <div className="odhKpiBody">
          ...
          <div className="odhKpi">
            <div className="odhKpiIcon odhKpiIcon-blue"><TruckIcon /></div>
            <div className="odhKpiBody">
          ...
          <div className="odhKpi">
            <div className="odhKpiIcon odhKpiIcon-green"><CheckIcon /></div>
            <div className="odhKpiBody">
          ...
          <div className="odhKpi odhKpi-highlight">
            <div className="odhKpiIcon odhKpiIcon-red"><WalletIcon /></div>
            <div className="odhKpiBody">
              <div className="odhKpiLabel">Tổng tền mua</div>
              <div className="odhKpiValue" style={{ fontSize: 15 }}>{fmtVnd(kpis.spend)}đ</div>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="odhKpis">
          {Array.from({ length: 5 }).map((_, i) => (
            <div key={i} className="odhKpi">
              ...
              <div style={{ flex: 1 }}>
                <Skeleton width="90px" height="12px" style={{ marginBottom: 6 }} />
      ...
      {!loading && !error && rows.length > 0 && pager.last > 1 && (
        <div className="odPager">
          ...
          {pageNumbers.map((n, i) =>
            n < 0 ? (
              <span key={`ellipsis-${i}`} className="odPageInfo">…</span>
            ) : (
              <button
                key={n}
                type="button"
      ...
      <div className={inProfile ? 'odhTop' : 'odTop'}>
        <div>
          ...
          <div className={inProfile ? 'odhSub' : 'odSub'}>
            Nơi lưu trữ toàn bộ đơn hàng của bạn. Dễ dàng kiểm tra tiến trình giao hàng, xem chi tiết hóa đơn, gửi yêu cầu đổi trả hoặc mua lại sản phẩm chỉ với vài thao tác.
          </div>
        </div>
