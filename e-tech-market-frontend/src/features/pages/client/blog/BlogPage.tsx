...
export default function BlogPage() {
  ...
  const isFavorited = (post: BlogPost) => {
    ...
    if (post.isProductNews) {
      ...
      const realId = post.id.toString().startsWith("999")
        ? parseInt(post.id.toString().substring(3))
        : post.id;
  ...
  useEffect(() => {
    ...
    Promise.all([
      apiFetch<{ data: BlogPost[] }>("/api/blog/posts").catch(() => ({
        data: [],
      })),
      ...
      .then(([postsRes, newsRes]) => {
        if (active) {
          ...
          const productNews: BlogPost[] = (newsRes.data || []).map(
            (news: any) => ({
              id: parseInt(`999${news.id}`), // Prevent ID collision
              title: news.title,
              slug: news.slug,
              excerpt: news.content_html
                ? news.content_html.replace(/<[^>]+>/g, "").substring(0, 120) +
                  "..."
  ...
  const submitNewsletter = async () => {
    ...
    try {
      ...
      setNewsletterMessage(
        e instanceof Error ? e.message : "Khong dang ky duoc email.",
      );
    } finally {
      ...
      <div className="ppContainer">
        ...
        {featuredPost && activeFilter === "all" && (
          <div className="blogHero">
            <img
              src={resolveImageUrl(featuredPost.thumbnail_url)}
              alt={featuredPost.title}
              className="blogHeroImg"
            />
            <div className="blogHeroContent">
              <span
                className="blogHeroTag"
                style={{
                  background: getCategoryColor(featuredPost.category?.slug),
                }}
              >
              ...
              <h1 className="blogHeroTitle">
                <Link
                  to={
                    featuredPost.isProductNews
                      ? `/product-news/${featuredPost.slug}`
        ...
        <div className="blogMainGrid">
          <div className="blogContentArea">
            ...
            <div className="blogPostsGrid">
              {(activeFilter === "all" ? recentPosts : filteredPosts).map(
                (post) => (
                    ...
                    <div className="blogCardThumbWrap">
                      ...
                      {post.category && (
                        <span
                          ...
                          style={{
                            background: getCategoryColor(post.category.slug),
                          }}
                        >
                          {post.category.name}
                        </span>
          ...
          <aside className="blogSidebar">
            <div className="blogSidebarWidget blogNewsletter">
              ...
              <h3 className="blogNewsletterTitle">
                Đăng ký nhận tin công nghệ mới nhất
              </h3>
              <p className="blogNewsletterDesc">
                Cập nhật những đánh giá, xu hướng và mẹo hay công nghệ hàng tuần
                vào hộp thư của bạn.
              </p>
              <input
                type="email"
              ...
              {newsletterMessage && (
                <span className="blogNewsletterNote">{newsletterMessage}</span>
              )}
              <span className="blogNewsletterNote">
              ...
              <div className="blogTrendingList">
                {trendingPosts.map((post) => (
                  <Link
                    ...
                    className="blogTrendingItem"
                    key={post.id}
                  >
                    <img
                      src={resolveImageUrl(post.thumbnail_url)}
                      alt={post.title}
                      className="blogTrendingThumb"
                    />
                    <div className="blogTrendingInfo">
                      <h4 className="blogTrendingTitle">{post.title}</h4>
                      <span className="blogTrendingViews">
