...
export default function BlogPostDetailPage() {
    ...
    return (
      <div className="postPage">
        <div className="ppContainer" style={{ padding: '100px 0', textAlign: 'center' }}>
          ...
          <Link to="/blog" className="adminBtnPrimary" style={{ display: 'inline-block', marginTop: '20px', textDecoration: 'none' }}>
            Quay lại trang Blog
          </Link>
        </div>
      </div>
  ...
  const submitComment = async () => {
    ...
    try {
      const created = await apiFetch<BlogComment>(`/api/blog/posts/${slug}/comments`, {
        method: 'POST',
        token,
        body: JSON.stringify({
      ...
      setPost((current) => current
        ? {
          ...
          comments_count: (current.comments_count ?? current.comments?.length ?? 0) + 1,
        }
        : current)
      setCommentContent('')
      setCommentMessage('Bình luận đã được gửi.')
    } catch (e) {
      setCommentMessage(e instanceof Error ? e.message : 'Không gửi được bình luận.')
    } finally {
    ...
    <div className="postPage">
      ...
      <div className="ppContainer">
        <div className="postBreadcrumb">
          <Link to="/">Trang chủ</Link> • <Link to="/blog">Tin tức</Link> • <span>{post.title}</span>
        </div>

        <div className="postMainGrid">
          <div className="postMainContent">
            {post.category && <span className="postCategory">{post.category.name}</span>}
              ...
              <div className="postMetaItem">
                {post.author?.avatar_url ? (
                  <div className="authorMiniAvatar">
                    <img src={resolveImageUrl(post.author.avatar_url)} alt={post.author.name} />
                  </div>
                ) : (
            ...
            <div className="postShare">
              <div className="postShareButtons">
                <span style={{ fontSize: '14px', fontWeight: 700, color: '#0f172a', marginRight: '10px' }}>Chia sẻ:</span>
            ...
            <div className="commentsSection">
              <h3 className="commentCount">Bình luận ({post.comments_count ?? post.comments?.length ?? 0})</h3>
              <div className="commentList">
                {(post.comments ?? []).map((comment) => (
                  <div className="commentItem" key={comment.id}>
                    <div className="commentAvatar">
                      {comment.user?.avatar_url ? (
                        <img src={resolveImageUrl(comment.user.avatar_url)} alt={comment.author_name} className="commentAvatarImg" />
                      ) : (
                    ...
                    <div className="commentBody">
                      <div className="commentHead">
                        ...
                        <span className="commentDate">{new Date(comment.created_at).toLocaleString('vi-VN')}</span>
                      </div>
                ...
                {canComment ? (
                  <>
                    <p className="commentText">
                      Bình luận với tài khoản {storedUser?.name || storedUser?.email || 'đang đăng nhập'}.
                    </p>
                    <textarea
                    ...
                    />
                    <button className="commentSubmit" disabled={commentSubmitting} onClick={() => void submitComment()}>
                      {commentSubmitting ? 'Đang gửi...' : 'Gửi bình luận'}
                    </button>
                  </>
                ) : (
                  <Link to="/login" className="commentSubmit" style={{ display: 'inline-block', textDecoration: 'none' }}>
                    Đăng nhập để bình luận
                  </Link>
                )}
                {commentMessage && <p className="commentText">{commentMessage}</p>}
              </div>
            </div>
          </div>
          ...
          <aside className="postSidebar">
            <div className="postSidebarWidget blogNewsletter">
              ...
              <p className="blogNewsletterDesc">Cập nhật tin công nghệ mới nhất hàng tuần.</p>
            ...
            <div className="postSidebarWidget">
              ...
              <div className="postTagCloud">
                <Link to="/blog" className="postTag">Công nghệ</Link>
                <Link to="/blog" className="postTag">Đánh giá</Link>
                <Link to="/blog" className="postTag">Mẹo hay</Link>
                <Link to="/blog" className="postTag">Xu hướng</Link>
              </div>
            </div>
          </aside>
        </div>
