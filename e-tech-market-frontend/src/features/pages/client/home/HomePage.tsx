...
import {
  fetchActiveBanners,
  type Banner,
} from "@/features/services/client/banners.client.service";
import {
  useWishlistQuery,
  useWishlistMutation,
...
function ProductCard({
  ...
  const excerpt =
    ...
    "Thiết kế thông minh, lực hút mạnh mẽ, làm sạch hoàn hảo mọi ngóc ngách trong ngôi nhà của bạn...";
  ...
  const [isInCompare, setIsInCompare] = useState(() =>
    getCompareList().some((p) => p.id === product.id),
  );

  useEffect(() => {
    const handleCompareChange = () =>
      setIsInCompare(getCompareList().some((p) => p.id === product.id));
    window.addEventListener("compare-change", handleCompareChange);
    return () =>
      window.removeEventListener("compare-change", handleCompareChange);
  }, [product.id]);
      ...
      <div className="hpProductImageWrap">
        ...
        <div className="hpProductActions">
          ...
          <button
            ...
            style={{
              color: isInCompare ? "#f97316" : undefined,
              borderColor: isInCompare ? "#f97316" : undefined,
            }}
          >
            <ExpandIcon />
          </button>
        ...
        {!isFlashSale && discountPercent > 0 ? (
          <span className="hpBadge hpBadgeSale">-{discountPercent}%</span>
      ...
      <div className="hpProductInfo">
        ...
        <div className="ppCardPriceRow">
          ...
          {displayOldPrice &&
            ...
            showDiscountBadge && (
              <span className="ppCardOldPrice">
                {displayOldPrice.toLocaleString("vi-VN")} đ
              </span>
            )}
        </div>
...
function Stars({ value }: { value: number }) {
  ...
  return (
    <div className="hpStars">
      {arr.map((t, idx) => (
        <span key={idx} className={`hpStar hpStar--${t}`}>
          ★
        </span>
      ))}
    </div>
...
export default function HomePage() {
  ...
  const [tabActive, setTabActive] = useState<
    "phone" | "laptop" | "pc" | "monitor" | "printer"
  >("phone");
  ...
  const wishSet = useMemo(
    () => new Set(wishlistData?.map((i) => i.product_id) || []),
    [wishlistData],
  );
        ...
        ([
          ...
          if (active) {
            ...
            if (Array.isArray(catRes)) {
              const mainCats = catRes
                .filter((c) => c.parent_id === null && c.is_active && c.image)
                .slice(0, 5);
            ...
            if (newsRes && Array.isArray(newsRes.data)) {
              ...
              const productNews: BlogPost[] = (productNewsRes?.data || []).map(
                (news: any) => ({
                  id: parseInt(`999${news.id}`),
                  title: news.title,
                  slug: news.slug,
                  excerpt: news.content_html
                    ? news.content_html
                        .replace(/<[^>]+>/g, "")
                        .substring(0, 120) + "..."
              ...
              const allPosts = [...blogPosts, ...productNews].sort(
                (a, b) =>
  ...
  const reviewMediaModal =
    selectedReviewMediaList.length > 0
      ? createPortal(
          <div
            className="hpReviewMediaModalOverlay"
          ...
          >
            ...
            >
              ...
              <div className="hpReviewMediaModalContent">
                {selectedReviewMediaList[selectedReviewMediaIndex]?.type ===
                "image" ? (
                  <img
                    src={resolveImageUrl(
                      selectedReviewMediaList[selectedReviewMediaIndex]?.url,
                    )}
                    alt="Ảnh đánh giá"
                  />
                ) : (
              ...
              {selectedReviewMediaList.length > 1 && (
                <div className="hpReviewMediaModalNav">
                  <button
                    ...
                    onClick={() =>
                      setSelectedReviewMediaIndex((current) =>
                  ...
                  <span className="hpReviewMediaModalNavCounter">
                    {selectedReviewMediaIndex + 1} /{" "}
                    {selectedReviewMediaList.length}
                  </span>
                  <button
                    type="button"
                    className="hpReviewMediaModalNavBtn"
      ...
      <main className="hpMain">
          ...
          <section className={`hpHeroNew ${isLightBg ? "theme-light" : ""}`}>
            ...
            <div className="hpHeroContent">
              {banners.length > 1 && (
                <>
                  <button
                    className="hpBannerNavBtn hpBannerPrev"
                    onClick={() =>
                      setCurrentBannerIndex((i) =>
                        i === 0 ? banners.length - 1 : i - 1,
                      )
                    }
                  >
                    ‹
                  ...
                  <button
                    className="hpBannerNavBtn hpBannerNext"
                    onClick={() =>
                      setCurrentBannerIndex((i) =>
                        i === banners.length - 1 ? 0 : i + 1,
                      )
                    }
                  >
                    ›
              ...
              <div className="hpHeroTextWrapper">
                ...
                <div className="hpHeroActions">
                  <button
                    ...
                    onClick={() =>
                      navigate(
                        banners[currentBannerIndex]?.link_url || "/products",
                      )
                    }
                  >
                    KHÁM PHÁ NGAY &rarr;
            ...
            <div className="hpHeroFeatures">
              <div className="hpHeroMarqueeTrack">
                ...
                {[
                  ...
                  <div
                    key={`dup-${idx}`}
                    className="hpFeatureItem hpFeatureItemDup"
                  >
                    <div className="hpFeatureIcon">{f.icon}</div>
                    <div className="hpFeatureText">{f.text}</div>
                  </div>
          ...
          <section className="hpHeroNew">
            ...
            <div className="hpHeroContent">
              <div className="hpHeroTextWrapper">
                ...
                <h1 className="hpHeroTitleNew">
                  <span className="text-white">CHẠM ĐẾN TƯƠNG LAI</span>
                  <br />
                  <span className="text-orange">THIẾT BỊ ĐỈNH CAO</span>
                </h1>

                <p className="hpHeroDescNew">
                  Khám phá thế hệ công nghệ mới với những thiết bị chính hãng,
                  hiệu năng vượt trội và thiết kế hiện đại - đáp ứng mọi nhu cầu
          ...
          (() => {
            ...
            const visibleCoupons = hasAuth
              ? activeCoupons.filter((c) => {
                  if (
                    c.max_uses_per_user &&
                    c.user_usage_count >= c.max_uses_per_user
                  )
                    return false;
                  return true;
                })
              ...
              <section className="hpCouponSection reveal">
                <div className="hpContainer">
                  <div className="hpCouponHeader">
                    <div>
                      <h2 className="hpCouponSectionTitle">
                        Ưu đãi dành cho bạn
                      </h2>
                      <p className="hpCouponSectionSub">
                  ...
                  <div className="hpCouponGridWrapper">
                    <div className="hpCouponGrid">
                      {loading
                        ...
                        : [...visibleCoupons, ...visibleCoupons].map(
                            (c, index) => {
                              ...
                              return (
                                <div
                                  key={`${c.id}-${index}`}
                                  className="hpCouponCardNew"
                                >
                                  <div className="hpCouponCardNewLeft">
                                    <span className="hpCouponCardNewLeftLabel">
                                      Giảm
                                    </span>
                                    <span className="hpCouponCardNewLeftValue">
                                      {c.coupon_type === "percentage" ? (
                                        ...
                                        <>
                                          {Math.floor(c.value / 1000)}
                                          <span className="percent">K</span>
                                        </>
                                      )}
                                    </span>
                                    ...
                                    <div className="hpCouponCardNewRight">
                                      <div className="hpCouponCardNewRightTop">
                                        <div className="hpCouponCardNewIconBox">
                                          ...
                                          >
                                            <path
                                              d="M21 10V6C21 4.9 20.1 4 19 4H5C3.9 4 3 4.9 3 6V10C4.1 10 5 10.9 5 12C5 13.1 4.1 14 3 14V18C3 19.1 3.9 20 5 20H19C20.1 20 21 19.1 21 18V14C19.9 14 19 13.1 19 12C19 10.9 19.9 10 21 10Z"
                                              stroke="#FF6B00"
                                              stroke-width="2"
                                        ...
                                        <div className="hpCouponCardNewInfo">
                                          <div className="hpCouponCardNewTitle">
                                            {c.coupon_type === "percentage"
                                              ? `Giảm ${c.value}%`
                                              : `Giảm ${formatPriceVnd(c.value.toString())}`}
                                          </div>
                                          <div className="hpCouponCardNewMin">
                                            {c.min_order_amount
                                              ? `Đơn từ ${formatPriceVnd(c.min_order_amount.toString())}`
                                            ...
                                            <div className="hpCouponCardNewUsageRow">
                                              ...
                                              <div className="hpCouponCardNewUsageBar">
                                                <div
                                                  ...
                                                  style={{
                                                    width: `${Math.max(0, Math.min(100, ((showLimit - showRemaining) / showLimit) * 100))}%`,
                                                  }}
                                                />
                                              </div>
                                            </div>
                                      ...
                                      <div
                                        className="hpCouponCardNewCodeBox"
                                        onClick={() => {
        ...
        <section className="hpCuratedSection reveal">
          <div className="hpContainer">
            ...
            <div className="hpCuratedGrid">
              {loading ? (
                <>
                  <div className="hpCuratedCard hpCuratedCard--1">
                    <CategorySkeleton />
                  </div>
                  <div className="hpCuratedCard hpCuratedCard--2">
                    <CategorySkeleton />
                  </div>
                  <div className="hpCuratedCard hpCuratedCard--3">
                    <CategorySkeleton />
                  </div>
                  <div className="hpCuratedCard hpCuratedCard--4">
                    <CategorySkeleton />
                  </div>
                  <div className="hpCuratedCard hpCuratedCard--5">
                    <CategorySkeleton />
                  </div>
                </>
              ) : categories.length > 0 ? (
        ...
        <section className="hpFeaturedSection reveal">
          <div className="hpContainer">
            <div className="hpSectionHeaderNew">
              <h1 className="hpSectionTitleNew text-center mb-5">
                Hàng mới &amp; Nổi bật
              </h1>
            </div>
            ...
            <div className="hpFutureGrid">
              <div className="hpFutureContent">
                ...
                <p className="hpFutureDesc">
                  Chúng tôi không chỉ bán thiết bị điện tử — chúng tôi chọn lọc
                  những công cụ thúc đẩy sự tiến bộ. Quy trình tuyển chọn dựa
                  trên kiểm định kỹ thuật khắt khe để mỗi sản phẩm đạt tiêu
                  chuẩn “Chất lượng không thỏa hiệp”.
                </p>
        ...
        {latestNews.length > 0 && (
          <section className="hpNewsSection reveal">
            <div className="hpContainer">
              <div className="hpSectionHeaderNew">
                <span className="hpSectionKicker text-center">
                  TIN TỨC CÔNG NGHỆ
                </span>
                <h2 className="hpSectionTitleNew text-center mb-5">
                  Bài viết mới nhất
                </h2>
              </div>
        ...
        {homeVideos.length > 0 && (
          <section
            className="hpCuratedSection reveal"
          ...
          <div className="hpContainer">
            <div className="hpWhyUsGrid">
              <div className="hpWhyUsItem">
                ...
                <p>
                  Miễn phí giao hàng toàn quốc cho đơn từ 5 triệu đồng, giao
                  nhanh và đảm bảo sản phẩm nguyên vẹn.
                </p>
              </div>
              <div className="hpWhyUsItem">
                <div className="hpWhyIcon">
                ...
                <p>
                  Bảo hành chính hãng lên đến 24 tháng, hỗ trợ đổi trả theo
                  chính sách và xử lý nhanh khi phát sinh lỗi.
                </p>
              </div>
              <div className="hpWhyUsItem">
                <div className="hpWhyIcon">
                ...
                <p>
                  Chuyên viên kỹ thuật tư vấn cấu hình, nâng cấp và lắp đặt PC,
                  luôn đồng hành cùng bạn trước và sau khi mua hàng.
                </p>
              </div>
              <div className="hpWhyUsItem">
                <div className="hpWhyIcon">
                ...
                <p>
                  Thanh toán linh hoạt qua thẻ, chuyển khoản hoặc COD với hệ
                  thống bảo mật hiện đại, đảm bảo an toàn cho mọi giao dịch.
                </p>
              </div>
            </div>
          </div>
                  ...
                  <div key={rev.id} className="hpReviewCard">
                    <div className="hpReviewLeft">
                      <div className="hpReviewAvatar">
                        {rev.user?.avatar_url ? (
                          <img
                            src={resolveImageUrl(rev.user.avatar_url)}
                            alt={rev.user.name}
                          />
                        ) : (
                      ...
                      <div className="hpReviewUserName">
                        ...
                        >
                          {rev.media.map((item, idx) => (
                            <button
                              ...
                              onClick={() => {
                                const mediaList = Array.isArray(rev.media)
                                  ? rev.media
                                  : [];
                                setSelectedReviewMediaList(
                                  mediaList.map((m) => ({
                                    url: m.url,
                                    type: m.type,
                                  })),
                            ...
                            >
                              {item.type === "image" ? (
                                ...
                                <div className="hpReviewMediaVideoWrap">
                                  ...
                                  />
                                  <span
                                    className="hpReviewMediaVideoBadge"
                                    aria-hidden
                                  >
                                    ▶
                                  </span>
          ...
          <div className="hpContainer">
            <div className="hpNewsletterBox">
              ...
              <p className="hpNewsletterDesc">
                Tham gia cộng đồng ưu tiên: ưu đãi sớm cho phiên bản giới hạn và
                tài liệu kỹ thuật chọn lọc.
              </p>
