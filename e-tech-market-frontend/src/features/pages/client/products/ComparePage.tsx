...
export default function ComparePage() {
  const [compareList, setCompareList] =
    useState<CompareProduct[]>(getCompareList());
  ...
  const resolveSpecValue = (product: Product, group: string, key: string) => {
    const s = product.specs?.find(
      (spec) =>
        (spec.spec_group || "Thông tin khác") === group &&
        spec.spec_key === key,
    );
    if (!s) return "—";
    ...
    return (
      <div className="comparePage empty">
        <div className="compareEmptyContent">
          ...
          <Link to="/products" className="compareBackBtn">
            Quay lại cửa hàng
          </Link>
        </div>
      </div>
    ...
    <div className="comparePage">
      <div className="compareContainer">
        <div className="compareHeader">
          ...
          <p>
            Bảng so sánh chi tiết thông số kỹ thuật giữa các sản phẩm đã chọn.
          </p>
        </div>
                  ...
                  <Fragment key={groupName}>
                    ...
                    {Array.from(keys).map((key) => (
                      <tr key={key}>
                        ...
                        {products.map((p) => (
                          <td key={p.id}>
                            {resolveSpecValue(p, groupName, key)}
                          </td>
                        ))}
                      </tr>
                    ))}
