...
import "@/styles/pages/ProductsPage.desc-row.css";
...
function categoryTreeHasSelected(
...
function CategoryItem({
  ...
  const shouldOpen =
    hasChildren && categoryTreeHasSelected(category, selectedId);
    ...
    <div className="ppCatItemWrap">
      <div className="ppCatItemHeader">
        {!hasChildren ? (
          ...
          <div
            className={`ppCatParentName ${isOpen ? "active" : ""}`}
        ...
        {hasChildren && (
          <button
            className={`ppCatToggle ${isOpen ? "open" : ""}`}
            onClick={() => setIsOpen(!isOpen)}
          >
            ▾
          </button>
...
function ProductDescWithDetailButton({ product }: { product: ApiProduct }) {
  const description =
  ...
  useEffect(() => {
    ...
    const compute = () => {
      // Nếu có CSS line-clamp/fix thì lấy scrollHeight sẽ ổn định hơn
      const lineHeight = parseFloat(getComputedStyle(el).lineHeight);
      const h = el.scrollHeight;
      if (!Number.isFinite(lineHeight) || lineHeight <= 0) {
        setIsMultiLine(h > el.clientHeight + 1);
        return;
      }
      // line clamp: app Flutter chia theo 1 dòng vs phần còn lại.
      // Ở đây coi multi-line khi vượt ~1 line.
      setIsMultiLine(h > lineHeight * 1.35);
    };
  ...
  return (
    <div className="ppCardDescRow">
      <div ref={containerRef} className="ppCardDescClamp">
        {description}
      </div>

      {/* <Link to={`/products/${product.slug}`} className="ppCardAddBtn">
      →
      </Link> */}

      {isMultiLine && (
        <div className="ppCardDescSideCover" aria-hidden="true" />
      )}
    </div>
  );
}
    ...
    <div className="ppCardNew">
      ...
      >
        ...
        {(isNew || product.is_featured || discountPercent > 0) && (
          <div className="ppCardBadges">
            {product.is_featured && (
              <span className="ppCardBadge ppCardBadge--featured">NỔI BẬT</span>
            )}
            {isNew && <span className="ppCardBadge ppCardBadge--new">MỚI</span>}
            {discountPercent > 0 && (
              <span className="ppCardBadge ppCardBadge--discount">
                -{discountPercent}%
              </span>
            )}
          </div>
      ...
      <div className="ppCardContent">
        <div className="ppCardTopRow">
          ...
          <span
            className="ppCardRating"
            aria-label={`Đánh giá ${avgRating.toFixed(1)} trên 5`}
          >
            ...
            {ratingCount > 0 && (
              <span className="ppCardRatingText"> ({ratingCount})</span>
            )}
          </span>
        </div>
...
export default function ProductsPage() {
  ...
  useEffect(() => {
    setQuery((current) =>
      current === searchQueryParam ? current : searchQueryParam,
    );
    setPage(1);
  }, [searchQueryParam]);
    ...
    <div className="ppPageNew">
      ...
      <div className="ppContainer">
        <div className="ppMainGrid">
          <aside
            className={`ppSidebarNew ${showMobileFilters ? "ppSidebarNew--open" : ""}`}
          >
            <div
              className="ppSidebarClose"
              onClick={() => setShowMobileFilters(false)}
            >
              ✕
            </div>
          ...
          <div className="ppContentNew">
            <header className="ppHeaderNew">
              <div className="ppHeaderLeft">
                ...
                <p className="ppCategorySub">
                  Thiết bị công nghệ tuyển chọn với chất lượng chính hãng, hiệu
                  năng ổn định và thiết kế hiện đại. Đáp ứng tốt nhu cầu học
                  tập, làm việc, giải trí và gaming hằng ngày.
                </p>
              </div>
              <div className="ppHeaderRight">
                ...
                <div className="ppControlsRow">
                  <div className="ppSortWrap" ref={sortRef}>
                    ...
                    <div
                      className={`ppSortDropdownNew ${sortOpen ? "ppSortDropdownNew--open" : ""}`}
                    >
                      <button
                        type="button"
            ...
            {!loading && totalItems > 0 && (
              <div className="ppPagination">
                <div className="ppPaginationInfo">
                  Hiển thị <b>{pageStart}</b>–<b>{pageEnd}</b> /{" "}
                  <b>{totalItems}</b>
                </div>
                <div className="ppPaginationBtns">
                  ...
                  {pageNumbers[0] > 1 && (
                    <>
                      ...
                      {pageNumbers[0] > 2 && (
                        <span className="ppPageDots">…</span>
                      )}
                    </>
                  )}
