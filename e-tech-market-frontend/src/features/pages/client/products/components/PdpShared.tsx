...
export function renderVideoPlayer(videoUrl: string, title?: string | null) {
  ...
  const videoSrc = normalizedVideoUrl.startsWith("http")
    ? normalizedVideoUrl
    : `${API_BASE_URL}${normalizedVideoUrl.startsWith("/") ? normalizedVideoUrl : `/${normalizedVideoUrl}`}`;
  return (
    <video
      src={videoSrc}
      controls
...
export const SHOP_REPLY_AVATAR_SRC =
  (import.meta.env.VITE_SHOP_REPLY_AVATAR_URL as string | undefined)?.trim() ||
  "/logoEtech.png";

export async function fetchProductShopQnasPublic(
  slug: string,
): Promise<ProductShopQnaPublic[]> {
  try {
...
export function scrollToPdpShopQnaForm() {
  document
    .getElementById("pdp-shop-qna-form")
...
export function buildVariantFacetModel(
  variants: ProductVariant[],
): VariantFacetModel {
  const colors: string[] = [];
  ...
  const storages = [...stSet].sort(
    (a, b) => storageSortRank(a) - storageSortRank(b),
  );
    ...
    >
      <div className="pdpThumbCol">
        {mediaItems.map((item, i) => {
          const isActive =
            selectedImg === item.url || (!selectedImg && i === 0);
          const thumbUrl =
            ...
            >
              {thumbUrl ? (
                <img
                  ...
                  alt=""
                  style={{
                    objectFit: item.type === "video" ? "cover" : "contain",
                  }}
                />
              ) : (
                <div
...
export function PdpFacetVariantPicker({
  ...
  const selColor = selectedVariant
  ...
  const { visibleStorages, canToggleStorages } = useMemo(() => {
    ...
    // Only show storages that actually exist for selected color (selColor)
    const colorStorages: string[] = [];
    const seen = new Set<string>();

    for (const st of facet.storages) {
      const key = `${selColor}\0${storageKeyNorm(st)}`;
      if (facet.byColorStorage.has(key)) {
        const norm = storageKeyNorm(st);
        if (!seen.has(norm)) {
          seen.add(norm);
          colorStorages.push(st);
        }
      }
    }

    // Fallback: if nothing found, show all (prevents empty UI)
    const all = colorStorages.length > 0 ? colorStorages : facet.storages;
    const canToggle = all.length > MAX_COLLAPSED;

    if (!canToggle || showAllStorages)
      return { visibleStorages: all, canToggleStorages: canToggle };

    const selectedKey = storageKeyNorm(selStorage);
    const selectedInAll = all.find((s) => storageKeyNorm(s) === selectedKey);

    const base = all.slice(0, MAX_COLLAPSED - 1); // reserve slot for "Xem thêm"
    ...
    <div className="pdpFacetVariant">
            ...
            {facet.colors.map((col) => {
              ...
              return (
                <button
                  ...
                  onClick={() =>
                    onSelectVariant(
                      resolveVariantByFacet(facet, col, selStorage),
                    )
                  }
                  aria-pressed={active}
                >
                  <span className="pdpFacetColorChip__img">
                    <img
                      src={resolveImageUrl(
                        rep.image_url || product.main_image_url,
                      )}
                      alt=""
                    />
                  </span>
      ...
      {facet.storages.length > 0 && (
        <div className="pdpFacetSection">
          ...
          <div
            className="pdpFacetChipRow pdpFacetChipRow--storages"
            role="list"
          >
            {visibleStorages.map((st) => {
              ...
              // vOpt should exist if st is derived from colorStorages
              if (!vOpt) return null;

              const active = storageKeyNorm(st) === storageKeyNorm(selStorage);
              const priceStr = `${parseFloat((vOpt.effective_price ?? 0).toString()).toLocaleString("vi-VN")} đ`;
            ...
            {canToggleStorages && !showAllStorages && (
              ...
              >
                ...
                <span className="pdpFacetStorageChip__price">
                  phiên bản khác
                </span>
              </button>
            )}
...
export function Stars({ value }: { value: number }) {
  ...
  return (
    <span className="pdpStars">
      {stars.map((t, i) => (
        <span key={i} className={`pdpStar ${t}`}>
          ★
        </span>
      ))}
    </span>
...
export function IconPaperPlane({ className }: { className?: string }) {
  return (
    <svg
      className={className}
      viewBox="0 0 24 24"
      ...
      aria-hidden
    >
      <path fill="currentColor" d="m2 21 21-9L2 3v7l13 4-13 4v7z" />
    </svg>
  );
}
export function IconQnaChatBubble({ className }: { className?: string }) {
  return (
    <svg
      className={className}
      viewBox="0 0 24 24"
