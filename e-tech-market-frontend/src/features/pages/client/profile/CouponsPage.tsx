...
export default function CouponsPage() {
  ...
  const [tab, setTab] = useState<"all" | "available" | "expiring" | "used">(
    "all",
  );
  ...
  const isAvailable = (c: CouponPublic) => {
    ...
    const exhausted = c.max_uses_per_user
      ? (c.user_usage_count || 0) >= c.max_uses_per_user
      : false;
  ...
  const availableCount = useMemo(
    () => activeCoupons.filter(isAvailable).length,
    [activeCoupons],
  );
  const expiringCount = useMemo(
    () => activeCoupons.filter(isExpiring).length,
    [activeCoupons],
  );
  const usedCount = useMemo(
    () => activeCoupons.filter(isUsed).length,
    [activeCoupons],
  );
  ...
  if (loading) {
    return (
      <div style={{ padding: "20px 0", color: "var(--et-text-muted)" }}>
  ...
  const renderTicket = (
    ...
    const showRemaining =
      showLimit !== null
        ? Math.max(0, showLimit - (c.user_usage_count || 0))
        : null;

    return (
      <div
        key={`${c.id}-${index}`}
        className={`cpTicketCard ${variant} ${isDisabled ? "disabled" : ""}`}
      >
        <div className="cpTicketLeft">
          ...
          <div className="cpTicketLeftBottom">
            {c.min_order_amount
              ? `Đơn từ ${formatVnd(c.min_order_amount)}`
              : "Áp dụng mọi đơn"}
          </div>
        </div>
          ...
          {variant === "featured" ? (
            ...
            <>
                  ...
                  {c.end_at && (
                    <>
                      <div className="cpTicketMetaRow">
                        <ClockIcon /> HSD:{" "}
                        {new Date(c.end_at).toLocaleDateString("vi-VN")}
                      </div>
                      {daysRemaining !== null && daysRemaining > 0 && (
                        <div
                          className="cpTicketMetaRow"
                          style={{
                            color: daysRemaining <= 5 ? "#ef4444" : "#0db504ff",
                            fontWeight: 600,
                          }}
                        >
              ...
              <div className="cpTicketActionCol">
                ...
                {isDisabled ? (
                  <button className="cpTicketUseBtn" disabled>
                    ĐÃ SỬ DỤNG
                  </button>
                ) : (
                  <button
                    className="cpTicketUseBtn"
                    onClick={() => handleCopyCode(c.code)}
                  >
                    SỬ DỤNG
                  </button>
    ...
    <div className="cpRoot">
      ...
      <div className="cpSummaryGrid">
        <div className="cpSummaryCard">
          <div className="cpSummaryIcon orange">
            <TicketIcon />
          </div>
        ...
        <div className="cpSummaryCard">
          <div className="cpSummaryIcon green">
            <TicketIcon />
          </div>
          <div className="cpSummaryInfo">
        ...
        <div className="cpSummaryCard">
          <div className="cpSummaryIcon yellow">
            <ClockIcon />
          </div>
          <div className="cpSummaryInfo">
        ...
        <div className="cpSummaryCard">
          <div className="cpSummaryIcon blue">
            <RefreshIcon />
          </div>
          <div className="cpSummaryInfo">
      ...
      {activeCoupons.length === 0 ? (
        <div className="cp-empty-state">
          ...
          <p className="cp-empty-desc">
            Rất tiếc, hiện tại bạn chưa có mã giảm giá nào. Hãy mua sắm ngay để
            có cơ hội nhận các voucher ưu đãi hấp dẫn!
          </p>
          ...
          {featuredCoupons.length > 0 && (
            <div>
              <div className="cpSectionTitleRow">
                <div className="cpSectionTitle">
                  <span>Voucher</span> nổi bật
                </div>
              </div>
