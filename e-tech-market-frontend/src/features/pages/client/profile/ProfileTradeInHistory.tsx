...
const STATUS_MAP: Record<string, { label: string; cls: string; filterKey: string }> = {
  ...
  const handleAcceptQuote = async (id: number) => {
    ...
    try {
      ...
      if (res.status === "success") {
        ...
        setSelectedReq(prev => prev?.id === id ? { ...prev, status: "approved" } : prev);
      }
    } catch (err: any) {
      ...
      <div className="ptih-container">
        <div className="ptih-header">
          <div>
            ...
            <p className="ptih-subtitle">Thu máy cũ giá tốt – Lên đời dễ dàng – Trợ giá hấp dẫn</p>
          </div>
        </div>
    ...
    <div className={`ptih-container ${selectedReq ? "ptih-has-detail" : ""}`}>
      ...
      <div className="ptih-layout">
              ...
              {totalPages > 1 && (
                <div className="ptih-pagination">
                  <span className="ptih-page-info">
                    Hiển thị {(page - 1) * PAGE_SIZE + 1}–{Math.min(page * PAGE_SIZE, filtered.length)} trong {filtered.length} yêu cầu
                  </span>
        ...
        {selectedReq && (() => {
          ...
          const statusInfo = STATUS_MAP[selectedReq.status] ?? { label: selectedReq.status, cls: "gray", filterKey: "" };
          const hasPrice   = !!selectedReq.estimated_price;
          const hasNote    = !!selectedReq.admin_note;
          const SHOW_MAX   = 5;
          const extraCount = imgs.length > SHOW_MAX ? imgs.length - SHOW_MAX : 0;
          ...
          return (
            <div className="ptih-detail-panel">
              ...
              <div className="ptih-detail-header">
                ...
                <button className="ptih-detail-close" onClick={() => setSelectedReq(null)}>
                  <IconClose />
                </button>
              </div>
              ...
              <div className="ptih-detail-status-bar">
                <span className={`ptih-status-badge-large ptih-badge-${statusInfo.cls}`}>
              ...
              <div className="ptih-detail-body">
                ...
                <div className="ptih-detail-section">
                  <div className="ptih-section-heading">
                    <span className="ptih-section-icon blue"><IconDevice /></span>
                    Thông tin thiết bị
                  </div>
                  <div className="ptih-info-table">
                    {name && (
                      <div className="ptih-info-row">
                        ...
                        <span className="ptih-info-val ptih-info-val-strong">{name}</span>
                      </div>
                    )}
                    <div className="ptih-info-row">
                      <span className="ptih-info-key">Mã yêu cầu</span>
                      <span className="ptih-info-val ptih-code-val">#{selectedReq.request_code}</span>
                    </div>
                    {selectedReq.serial_number && (
                      <div className="ptih-info-row">
                        <span className="ptih-info-key">S/N</span>
                        <span className="ptih-info-val">{selectedReq.serial_number}</span>
                      </div>
                    )}
                    {specs.filter(s => s.key).map((s, i) => (
                    ...
                    <div className="ptih-info-row">
                      ...
                      <span className="ptih-info-val">
                        {new Date(selectedReq.created_at).toLocaleDateString("vi-VN")} •{" "}
                        {new Date(selectedReq.created_at).toLocaleTimeString("vi-VN", { hour: "2-digit", minute: "2-digit" })}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                {selectedReq.conditions && selectedReq.conditions.length > 0 && (
                  <div className="ptih-detail-section">
                    <div className="ptih-section-heading">
                      <span className="ptih-section-icon orange"><IconCondition /></span>
                      Tình trạng thiết bị
                    </div>
                    <div className="ptih-conditions-grid">
                      {selectedReq.conditions.map((c, i) => (
                        <div key={i} className="ptih-cond-row">
                          <span className="ptih-cond-check"><IconCheck /></span>
                          <span className="ptih-cond-label">{c.name.split(":")[0]?.trim()}</span>
                          {c.name.includes(":") && (
                            <span className="ptih-cond-val">{c.name.split(":").slice(1).join(":").trim()}</span>
                          )}
                        </div>
                      ))}
                    </div>
                ...
                <div className="ptih-detail-section">
                  <div className="ptih-section-heading">
                    <span className="ptih-section-icon green"><IconImage /></span>
                    Hình ảnh thiết bị
                  </div>
                  {imgs.length > 0 ? (
                      ...
                      {imgs.slice(0, SHOW_MAX).map((img, i) => (
                        ...
                        >
                          ...
                          {i === SHOW_MAX - 1 && extraCount > 0 && (
                            <div className="ptih-img-overlay">+{extraCount}</div>
                          )}
                        </div>
                      ))}
                    </div>
                  ) : (
                    <p className="ptih-no-image-text">Không có hình ảnh đính kèm.</p>
                  )}
                </div>
                ...
                {hasNote && (
                  <div className="ptih-detail-section">
                    <div className="ptih-section-heading">
                      <span className="ptih-section-icon amber"><IconNote /></span>
                      Ghi chú từ cửa hàng
                    </div>
                    <div className="ptih-note-box">{selectedReq.admin_note}</div>
                  </div>
                )}
                  ...
                  <div className="ptih-detail-section ptih-action-section">
                    <div className="ptih-notice-box">
                      ...
                      <span>
                        ...
                        Vui lòng mang máy ra <strong>cửa hàng gần nhất</strong> để nhân viên kiểm tra thực tế.
                      </span>
                    </div>
                    <button
                ...
                {selectedReq.status === "approved" && (
                  <div className="ptih-detail-section">
                    <div className="ptih-notice-success">
                      <IconCheck />
                      Bạn đã xác nhận mức giá. Vui lòng <strong>mang máy ra cửa hàng</strong> để hoàn tất thu mua.
                    </div>
                  </div>
                )}
                ...
                {selectedReq.status === "rejected" && (
                  <div className="ptih-detail-section">
                    <div className="ptih-notice-rejected">
                      ...
                      Yêu cầu này đã bị từ chối.{hasNote ? " Vui lòng xem ghi chú từ cửa hàng ở trên." : ""}
                    </div>
                  </div>
                )}
              </div>
      ...
      {lightbox && (
        ...
        >
          <img
            ...
            style={{ maxWidth: "90vw", maxHeight: "90vh", borderRadius: 8, objectFit: "contain", boxShadow: "0 25px 60px rgba(0,0,0,0.5)" }}
          />
          <button
            onClick={() => setLightbox(null)}
            style={{
              ...
              alignItems: "center", justifyContent: "center", color: "white", cursor: "pointer", backdropFilter: "blur(4px)",
            }}
          >
            <IconClose />
          </button>
