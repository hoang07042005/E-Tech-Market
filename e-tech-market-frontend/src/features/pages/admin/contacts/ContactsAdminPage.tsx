...
const GMAIL_COMPOSE_BASE_URL =
  "https://mail.google.com/mail/u/0/#inbox?compose=new";

export default function ContactsAdminPage() {
  const [messages, setMessages] = useState<ContactMessage[]>([]);
  const [loading, setLoading] = useState(true);
  const [handledFilter, setHandledFilter] = useState<"all" | "yes" | "no">(
    "all",
  );
  ...
  const visiblePages = useMemo(() => {
    ...
    const pages = [page - 1, page, page + 1].filter(
      (p) => p > 0 && p <= totalPages,
    );
    ...
    <div className="contactsAdminPage">
      ...
      <div className="contactsAdminHeader">
        ...
        <p className="adminPageSubtitle">
          Tiếp nhận, xử lý và phản hồi các yêu cầu từ khách hàng gửi về hệ
          thống.
        </p>
      </div>
      ...
      <div className="contactsToolbar">
        <div className="contactsFilterTabs">
          <button
            className={handledFilter === "all" ? "isActive" : ""}
            onClick={() => {
          ...
          <button
            className={handledFilter === "no" ? "isActive" : ""}
            onClick={() => {
              setHandledFilter("no");
              setPage(1);
            }}
          ...
          <button
            className={handledFilter === "yes" ? "isActive" : ""}
            onClick={() => {
              setHandledFilter("yes");
              setPage(1);
            }}
          >
            Đã giải quyết
      ...
      <div className="adminTableWrap">
        {loading ? (
          ...
          <div className="adminEmpty">
            Hộp thư trống. Không có tin nhắn nào khớp với bộ lọc.
          </div>
        ) : (
          <table className="contactsTable">
            <thead>
                ...
                >
                  ...
                  <td onClick={(e) => e.stopPropagation()}>
                    <div className="contactInfoCell">
                      <a className="contactInfoLink" href={`mailto:${m.email}`}>
                        {m.email}
                      </a>
                      {m.phone && (
                        <a className="contactInfoLink" href={`tel:${m.phone}`}>
                          {m.phone}
                        </a>
                      )}
                    </div>
                  ...
                  <td>
                    <p className="subjectText" title={m.subject}>
                      {m.subject}
                    </p>
                  </td>
                  <td>
                    <p className="messageText" title={m.message}>
                      {m.message}
                    </p>
                  </td>
                  <td>
                    <span
                      ...
                      title={
                        m.handled_at
                          ? `Xử lý bởi: ${m.handled_by?.name || "Admin"}`
                          : undefined
                      }
                    >
      ...
      <div className="contactsPagination">
        <p>
          ...
          trong tổng số <strong>{totalPages * 10}</strong> yêu cầu liên hệ
        </p>
        <div className="pageNumbers">
          ...
          {visiblePages.map((pageNumber) => (
            <button
              key={pageNumber}
              className={pageNumber === page ? "isCurrent" : ""}
      ...
      {selectedMessage && (
        <div
          className="contactDetailModalOverlay"
          onClick={() => setSelectedMessage(null)}
        >
          <div
            className="contactDetailModalBox"
            onClick={(e) => e.stopPropagation()}
          >
            ...
            <div className="modalContentLayout">
              ...
              <div className="modalSidebarInfo">
                <div className="modalAvatarLarge">
                  {getInitials(selectedMessage.name)}
                </div>
                <h4>{selectedMessage.name}</h4>
                <p className="modalMetaTime">
                  Gửi lúc:{" "}
                ...
                <div className="infoDetailRow">
                  ...
                  {selectedMessage.phone ? (
                    ...
                    <span className="infoValue italic text-muted">
                      Chưa cung cấp
                    </span>
                  )}
                </div>
                <div className="infoDetailRow">
                  ...
                  <span
                    className={`statusPill ${selectedMessage.handled_at ? "done" : "pending"}`}
                  >
                ...
                {selectedMessage.handled_at && (
                  <div className="handledMetaBox">
                    <p>
                      ...
                      {selectedMessage.handled_by?.name || "Quản trị viên"}
                    </p>
                    <p>
