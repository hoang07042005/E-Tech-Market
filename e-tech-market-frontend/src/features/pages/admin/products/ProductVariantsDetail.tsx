...
export default function ProductVariantsDetail({
      ...
      .catch((err) => {
        if (mounted) {
          if (err && typeof err === 'object' && 'message' in err) {
            setError(String((err as { message?: unknown }).message || 'Lỗi khi tải chi tiết sản phẩm.'))
          } else {
  ...
  if (isLoading) {
    return (
      <div className="prodAdminRoot" style={{ padding: '40px 0', textAlign: 'center' }}>
        <div className="et-loader-spinner" style={{ margin: '0 auto 20px' }}></div>
        <div style={{ color: '#64748b', fontSize: '15px', fontWeight: 600 }}>Đang tải thông số kỹ thuật & phiên bản...</div>
      </div>
    )
  }
    ...
    return (
      <div className="prodAdminRoot">
        <div className="prodHeader">
          <button className="pBackBtn" onClick={onBack} style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            <ArrowLeftIcon /> Quay lại
          </button>
        </div>
        <div className="prodErrorBanner">{error || 'Không tìm thấy thông tin sản phẩm.'}</div>
      </div>
    )
  }
  ...
  const outOfStockCount = variants.filter(v => (v.stock_quantity || 0) <= 0).length
  ...
  const minPrice = prices.length ? Math.min(...prices.map(p => p.effective)) : parseFloat(product.price || '0')
  const maxPrice = prices.length ? Math.max(...prices.map(p => p.effective)) : parseFloat(product.price || '0')
  ...
  const uniqueConfigs = uniqueColors.length === 0
    ? Array.from(new Set(variants.map(v => (v.configuration || '').trim()).filter(Boolean)))
    : []

  const filterType = uniqueColors.length > 0 ? 'color' : (uniqueConfigs.length > 0 ? 'config' : 'none')
  const filterItems = filterType === 'color' ? uniqueColors : uniqueConfigs

  // Filter variants displayed
  const displayedVariants = selectedFilter === 'all'
    ? variants
    : variants.filter(v => {
        if (filterType === 'color') return (v.color || '').trim() === selectedFilter
        if (filterType === 'config') return (v.configuration || '').trim() === selectedFilter
        return true
      })
    ...
    <div className="prodAdminRoot pDetailPremium">
      ...
      <div className="prodHeader pDetailHeader">
        <div className="pHeaderLeft">
          <button className="pBackBtn pBackDetailBtn" onClick={onBack} title="Quay lại danh sách">
            <ArrowLeftIcon />
            <span>Quay lại</span>
          </button>
          <div className="pHeaderMeta">
            ...
            <div className="pDetailBadges">
              <span className="pDetailBrand">{product.brand || 'No Brand'}</span>
              <span className="pDetailSep">•</span>
              <span className="pDetailCategory">{product.category?.name || 'Không có danh mục'}</span>
              <span className="pDetailSep">•</span>
              <span className={`pStatus ${product.is_active ? 'active' : 'inactive'}`}>
      ...
      <div className="prodStatsGrid pDetailStatsGrid">
        <div className="prodStatCard pDetailStatCard">
          <div className="prodStatTopRow">
            <div className="prodStatIcon prodStatIcon--orange"><BoxIcon /></div>
            <div className="prodStatHint">{activeVariants.length}/{variants.length} Đang kích hoạt</div>
          </div>
        ...
        <div className="prodStatCard pDetailStatCard">
          <div className="prodStatTopRow">
            <div className="prodStatIcon prodStatIcon--green"><DatabaseIcon /></div>
            {outOfStockCount > 0 ? (
              <div className="prodStatHint prodStatHint--red">{outOfStockCount} phiên bản hết hàng</div>
            ) : (
              <div className="prodStatHint prodStatHint--green">Tất cả ổn định</div>
            )}
          </div>
        ...
        <div className="prodStatCard pDetailStatCard">
          <div className="prodStatTopRow">
            <div className="prodStatIcon prodStatIcon--blue"><DollarIcon /></div>
            <div className="prodStatHint">Giá hiệu lực</div>
          </div>
          <div className="prodStatLabel">Giá thấp nhất</div>
          <div className="prodStatValue" style={{ fontSize: '24px', paddingTop: '10px' }}>
      ...
      <div className="pDetailGrid">
        ...
        <div className="pDetailMainContent">
              ...
              <div className="pVariantFilterTabs">
                ...
                {filterItems.map(item => {
                  const count = variants.filter(v => {
                    if (filterType === 'color') return (v.color || '').trim() === item
          ...
          {displayedVariants.length === 0 ? (
            <div className="pDetailEmptyBox">
              Sản phẩm này chưa có bất kì phiên bản (variants) nào. Vui lòng vào chỉnh sửa sản phẩm để thêm phiên bản.
            </div>
          ) : (
            <div className="pDetailVariantsList">
              {displayedVariants.map(variant => {
                const variantSpecs = variantSpecificSpecs.filter(s => s.product_variant_id === variant.id)
                const groupedVarSpecs = groupSpecs(variantSpecs)
                const originalPrice = parseFloat(variant.price)
                const finalPrice = variant.effective_price ? Number(variant.effective_price) : originalPrice
                  ...
                  <div key={variant.id} className="pDetailVariantCard">
                    ...
                    <div className="pVarCardHeader">
                      <div className="pVarCardImg">
                        <img 
                          src={variant.image_url ? resolveImageUrl(variant.image_url) : resolveImageUrl(product.main_image_url)} 
                      ...
                      <div className="pVarCardInfo">
                        <div className="pVarCardTitleWrap">
                          <h4 className="pVarCardTitle">{variant.variant_name}</h4>
                          <span className={`pStatus ${variant.is_active ? 'active' : 'inactive'}`} style={{ padding: '2px 8px', fontSize: '9px' }}>
                            {variant.is_active ? 'ĐANG KINH DOANH' : 'TẠM NGỪNG'}
                          </span>
                        </div>
                        ...
                        <div className="pVarCardMetaGrid">
                          <div className="pVarMetaItem">
                            ...
                            <span className="pVarMetaValue pSkuText">{variant.sku || '—'}</span>
                          </div>
                          <div className="pVarMetaItem">
                            <span className="pVarMetaLabel">Màu sắc:</span>
                            <span className="pVarMetaValue">{variant.color || '—'}</span>
                          </div>
                          <div className="pVarMetaItem">
                            <span className="pVarMetaLabel">Cấu hình:</span>
                            <span className="pVarMetaValue">{variant.configuration || '—'}</span>
                          </div>
                        </div>
                        ...
                        <div className="pVarCardPricingRow">
                          <div className="pVarPriceContainer">
                            {isDiscounted ? (
                              <>
                                <span className="pVarEffectivePrice">{formatCurrency(finalPrice)}</span>
                                <span className="pVarOriginalPrice">{formatCurrency(originalPrice)}</span>
                                <span className="pVarDiscountTag">
                                  Giảm {variant.discount_type === 'percentage' 
                                    ? `${Number(variant.discount_value)}%` 
                                    : formatCurrency(variant.discount_value)}
                                </span>
                              </>
                            ) : (
                              <span className="pVarEffectivePrice">{formatCurrency(originalPrice)}</span>
                            )}
                          </div>

                          <div className="pVarStockContainer">
                            <span className="pVarStockLabel">Kho hàng:</span>
                            <span className={`pVarStockValue ${(!variant.stock_quantity || variant.stock_quantity <= 0) ? 'out' : (variant.stock_quantity < 10) ? 'low' : 'ok'}`}>
                    ...
                    <div className="pVarCardSpecsSection">
                      <h5 className="pVarSpecsTitle">Thông số kĩ thuật riêng phiên bản</h5>
                      {groupedVarSpecs.length === 0 ? (
                        <p className="pVarSpecsEmpty">Không có thông số kĩ thuật riêng biệt cho phiên bản này.</p>
                      ) : (
                        <div className="pVarSpecsGrid">
                          {groupedVarSpecs.map(g => (
                            <div key={g.groupName} className="pVarSpecGroup">
                              <h6 className="pVarSpecGroupName">{g.groupName}</h6>
                              <table className="pVarSpecTable">
                                <tbody>
                                  {g.specs.map(spec => (
                                    <tr key={spec.id}>
                                      <td className="pSpecKey">{spec.spec_key}</td>
        ...
        <div className="pDetailSidebar">
          ...
          <div className="pSideSection">
            <h3 className="pBlockTitle flex-center"><SettingsIcon /> Thông số kĩ thuật chung</h3>
            {groupedGeneralSpecs.length === 0 ? (
              <div className="pSideSpecsEmpty">Không có thông số kĩ thuật dùng chung.</div>
            ) : (
          ...
          {/* Description & FAQs */}
          {(product.description || (product.faqs && product.faqs.length > 0)) && (
            <div className="pSideSection">
              ...
              {product.faqs && product.faqs.length > 0 && (
                <div className="pDetailFaqsWrapper">
                  <h4 className="pSideMiniTitle">Câu hỏi thường gặp ({product.faqs.length})</h4>
...
function ArrowLeftIcon() {return (<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><line x1="19" y1="12" x2="5" y2="12" /><polyline points="12 19 5 12 12 5" /></svg>)}
function BoxIcon() {return (<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z" /><polyline points="3.27 6.96 12 12.01 20.73 6.96" /><line x1="12" y1="22.08" x2="12" y2="12" /></svg>)}
function DatabaseIcon() {return (<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><ellipse cx="12" cy="5" rx="9" ry="3" /><path d="M3 5v14c0 1.66 4 3 9 3s9-1.34 9-3V5" /><path d="M3 12c0 1.66 4 3 9 3s9-1.34 9-3" /></svg>)}
function DollarIcon() {return (<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="12" y1="1" x2="12" y2="23" /><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6" /></svg>)}
