...
export default function ProductForm({ productId, onSave, onCancel }: ProductFormProps) {
    ...
    const fetchInitData = async () => {
      ...
      try {
        ...
        if (productId) {
          ...
          setVariants(((pData.variants ?? []) as any[]).map((v) => ({
            ...v,
  ...
  const addVariant = () => {
    setVariants([...variants, { variant_name: '', color: '', configuration: '', sku: '', price: '0', discount_type: '', discount_value: '', discount_start_at: '', discount_end_at: '', stock_quantity: 0 }])
  }
                    ...
                    {variants.map((v, index) => (
                      <div key={index} className="pVariantRowGroup">
                        <div className="pVariantRow">
                          ...
                          <div className="vField"><input placeholder="Tên phiên bản" value={v.variant_name} onChange={e => updateVariant(index, 'variant_name', e.target.value)} /></div>
                          <div className="vField"><input placeholder="Màu sắc" value={v.color} onChange={e => updateVariant(index, 'color', e.target.value)} /></div>
                          <div className="vField"><input placeholder="Cấu hình" value={v.configuration} onChange={e => updateVariant(index, 'configuration', e.target.value)} /></div>
                          <div className="vField"><input type="number" placeholder="Giá" value={v.price} onChange={e => updateVariant(index, 'price', e.target.value)} /></div>
                          <div className="vField"><input type="number" placeholder="Kho" value={v.stock_quantity} onChange={e => updateVariant(index, 'stock_quantity', parseInt(e.target.value))} /></div>
                          <button type="button" onClick={() => removeVariant(index)} className="pRemoveVariant">×</button>
                        </div>
                        ...
                        <div className="pVariantCard__discount">
                          ...
                          <div className="vDiscountFields">
                             <div className="vField">
                                <select value={v.discount_type || ''} onChange={e => updateVariant(index, 'discount_type', e.target.value)}>
                             ...
                             {v.discount_type && (
                               <>
                                 <div className="vField">
                                   <input type="number" placeholder="Mức giảm" value={v.discount_value || ''} onChange={e => updateVariant(index, 'discount_value', e.target.value)} />
                                 </div>
                                 <div className="vField">
                                   <input type="datetime-local" value={v.discount_start_at ? v.discount_start_at.slice(0, 16) : ''} onChange={e => updateVariant(index, 'discount_start_at', e.target.value)} />
                                 </div>
                                 <div className="vField">
                                   <input type="datetime-local" value={v.discount_end_at ? v.discount_end_at.slice(0, 16) : ''} onChange={e => updateVariant(index, 'discount_end_at', e.target.value)} />
                                 </div>
                               </>
                             )}
                          </div>
