...
export default function ProductForm({
  ...
  const variantStockSum = variants.reduce(
    (sum, v) =>
      sum + (Number.isFinite(v.stock_quantity) ? v.stock_quantity : 0),
    0,
  );
  ...
  useEffect(() => {
    const fetchInitData = async () => {
      ...
      try {
        ...
        if (productId) {
          const pData =
            await fetchAdminProductDetail<AdminProductDetail>(productId);
          setFormData({
            name: pData.name,
          ...
          setVariants(
            ((pData.variants ?? []) as any[]).map((v) => ({
              ...v,
              discount_type: v.discount_type || "",
              discount_value:
                v.discount_value != null ? String(v.discount_value) : "",
              discount_start_at: v.discount_start_at || "",
              discount_end_at: v.discount_end_at || "",
            })) as Variant[],
          );
          const invMain = pData.inventory_items?.find(
            (i) => i.location_code === "main",
          );
          setMainInventoryQty(
            typeof invMain?.quantity_on_hand === "number"
              ? invMain.quantity_on_hand
              : null,
          );
            ...
            setFaqs(
              rawFaqs.map(
                (f, i): ProductFaqForm => ({
                  ...
                  sort_order:
                    typeof f.sort_order === "number" ? f.sort_order : i,
  ...
  const handleReplaceAllProductImages = (
  ...
  const addFaq = () => {
    setFaqs([
      ...faqs,
      { question: "", answer: "", sort_order: faqs.length, is_active: true },
    ]);
  };

  const updateFaq = (
    index: number,
    field: keyof ProductFaqForm,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const resData = await saveAdminProduct<{
        inventory_items?: InventoryMainRow[];
      }>(data, productId);
      if (resData.inventory_items) {
        const invMain = resData.inventory_items.find(
          (i) => i.location_code === "main",
        );
        setMainInventoryQty(
          typeof invMain?.quantity_on_hand === "number"
  ...
  if (isLoading)
    return <div className="pFormLoading">Đang tải thông tin sản phẩm...</div>;

  return (
    <div className="pFormRoot">
      <div className="pFormHeader">
        <button className="pBackBtn" onClick={onCancel}>
          ← Quay lại danh sách
        </button>
        <h2 className="pFormTitle">
                  ...
                  <div className="pGeneralGrid">
                    <div className="pGeneralMain">
                      ...
                      <div className="pField">
                        ...
                        >
                          Mẹo: Hỗ trợ cả 2 định dạng: Dán mã HTML hoặc gõ văn
                          bản thông thường xuống dòng tự do (hệ thống sẽ tự nhận
                          diện và giữ nguyên định dạng hiển thị của bạn).
                        </p>
                      </div>
                    </div>
              ...
              <div className="pTabPane active">
                <div className="pFormSection" id="pf-images">
                  ...
                  <p className="pHint" style={{ marginBottom: "20px" }}>
                    Tải lên tối đa {MAX_PRODUCT_IMAGES} hình ảnh. Hình ảnh đầu
                    tiên sẽ là ảnh đại diện.
                    {productId && (
                      <span>
                        {" "}
                        Nếu đã đủ {MAX_PRODUCT_IMAGES} ảnh, dùng nút{" "}
                        <strong>Thay toàn bộ ảnh</strong> bên dưới để chọn bộ
                        ảnh mới rồi Lưu.
                      </span>
                    )}
                  </p>
                  <div className="pUploadGrid">
                    ...
                    {previews.length +
                      (selectedFiles.length > 0 ? 0 : existingImages.length) <
                      MAX_PRODUCT_IMAGES && (
                    ...
                    existingImages.length >= MAX_PRODUCT_IMAGES && (
                      <div className="pReplaceAllImages">
                        <p className="pHint pReplaceAllImages__text">
                          Bạn đang có đủ {MAX_PRODUCT_IMAGES} ảnh — nút
                          &quot;Thêm ảnh&quot; tạm ẩn. Bấm bên dưới để chọn tối
                          đa {MAX_PRODUCT_IMAGES} ảnh mới; khi Lưu, toàn bộ ảnh
                          cũ sẽ được thay bằng bộ này.
                        </p>
                  ...
                  {productId && selectedFiles.length > 0 && (
                    <p className="pWarningHint">
                      Lưu ý: Tải ảnh mới sẽ thay thế toàn bộ ảnh cũ của sản phẩm
                      này.
                    </p>
                  )}
                </div>
                ...
                <div className="pFormSection" id="pf-variants">
                  <div className="pInventorySyncNote">
                    ...
                    {productId && mainInventoryQty !== null && (
                      <>
                        ...
                        <span className="pInventoryHint">
                          {" "}
                          — đồng bộ khi Lưu sản phẩm.
                        </span>
                      </>
                    )}
                  </div>
                  <div className="pSectionHeader">
                    ...
                    <div className="pSpecActions">
                      <button
                        type="button"
                        className="pQuickPasteBtn"
                        onClick={() => setIsVariantPasteOpen(true)}
                      >
                        <PasteIcon /> Dán nhanh
                      </button>
                      <button
                        type="button"
                        className="pAddVariantBtn"
                        onClick={addVariant}
                      >
                        + Thêm phiên bản
                      </button>
                  ...
                  <div className="pVariantsWrapper">
                    <div className="pVariantsGrid">
                      {variants.length === 0 && (
                        <p className="pEmptySpecs">
                          Sản phẩm này chưa có phiên bản khác.
                        </p>
                      )}
                      ...
                      {variants.map((v, index) => (
                        <div key={index} className="pVariantRowGroup">
                          <div className="pVariantRow">
                              ...
                              <label className="vImgUpload">
                                ...
                                {v.imagePreview || v.image_url ? (
                                  <img
                                    src={
                                      v.imagePreview ||
                                      resolveImageUrl(v.image_url || null)
                                    }
                                    alt=""
                                  />
                                ) : (
                            ...
                            <button
                              ...
                              className="pRemoveVariant"
                            >
                              ×
                            </button>
                          </div>

                          <div className="pVariantCard__discount">
                            <div className="vDiscountBadge">KM</div>
                            <div className="vDiscountFields">
                              <div className="vField">
                                <select
                                ...
                                <>
                                  ...
                                  <div className="vField">
                                    <input
                                      ...
                                      value={
                                        v.discount_start_at
                                          ? v.discount_start_at.slice(0, 16)
                                          : ""
                                      }
                                      onChange={(e) =>
                                        updateVariant(
              ...
              <div className="pTabPane active">
                <div className="pFormSection" id="pf-specs">
                  <div className="pSectionHeader">
                    ...
                    <div className="pSpecActions">
                      <button
                        type="button"
                        className="pQuickPasteBtn"
                        onClick={() => setIsSpecPasteOpen(true)}
                      >
                        <PasteIcon /> Dán nhanh
                      </button>
                      <button
                        type="button"
                        className="pAddSpecBtn"
                        onClick={addSpec}
                      >
                        + Thêm thông số
                      </button>
                  ...
                  <div className="pSpecsGrid">
                    {specs.length === 0 && (
                      <p className="pEmptySpecs">Chưa có thông số nào.</p>
                    )}
                    ...
                    {specs.map((spec, index) => (
                      <div key={index} className="pSpecRow">
                        <select
                          ...
                          value={spec.scope}
                          onChange={(e) =>
                            updateSpec(index, "scope", e.target.value)
                          }
                        ...
                        >
                          ...
                          {variants.map((v, vi) => (
                            <option
                              key={vi}
                              value={v.id ? `variant:${v.id}` : `idx:${vi}`}
                            >
                              Chỉ:{" "}
                              {v.variant_name?.trim() || `Phiên bản ${vi + 1}`}
                            </option>
                          ))}
                        </select>
                        <input
                          ...
                          onChange={(e) =>
                            updateSpec(index, "spec_group", e.target.value)
                          }
                        />
                        <input
                          ...
                          value={spec.spec_key}
                          onChange={(e) =>
                            updateSpec(index, "spec_key", e.target.value)
                          }
                        />
                        <input
                          placeholder="Giá trị"
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            updateSpec(index, "spec_unit", e.target.value)
                          }
                          className="uInput"
                        />
                        <button
              ...
              <div className="pTabPane active">
                <div className="pFormSection" id="pf-faqs">
                  <div className="pSectionHeader">
                    ...
                    <div className="pSpecActions">
                      <button
                        type="button"
                        className="pQuickPasteBtn"
                        onClick={() => setIsFaqPasteOpen(true)}
                      >
                        <PasteIcon /> Dán nhanh
                      </button>
                      <button
                  ...
                  <div className="pFaqGrid">
                    {faqs.length === 0 && (
                      <p className="pEmptySpecs">
                        Thêm các câu hỏi thường gặp giúp khách tin tưởng hơn khi
                        xem sản phẩm.
                      </p>
                    )}
          ...
          <aside className="pOneTabSide">
            <div className="pSideCard">
              ...
              <div className="pSideHint">
                Mẹo: cuộn xuống theo từng khối; thanh lưu/hủy luôn nằm ở cuối
                (sticky).
              </div>
            </div>
          </aside>
        ...
        <div className="pModalOverlay">
          <div className="pModalContent small">
            ...
            <div className="pModalBody">
              <p className="pHint">
                Định dạng: <b>Nhóm | Tên | Giá trị | Đơn vị</b> (Mỗi thông số
                một dòng)
              </p>
              <textarea
            ...
            <div className="pModalFooter">
              <button
                className="pCancelBtn"
                onClick={() => setIsSpecPasteOpen(false)}
              >
                Hủy
              </button>
              <button className="pSubmitBtn" onClick={handleSpecQuickPaste}>
                Phân tích & Thêm
              </button>
            </div>
          </div>
        ...
        <div className="pModalOverlay">
          <div className="pModalContent small">
            <div className="pModalHeader">
              ...
              <button type="button" onClick={() => setIsFaqPasteOpen(false)}>
                ×
              </button>
            </div>
            <div className="pModalBody">
              <p className="pHint">
                Định dạng: <b>Câu hỏi | Trả lời</b> (mỗi cặp một dòng)
              </p>
              <textarea
                className="pQuickPasteArea"
                rows={10}
            ...
            <div className="pModalFooter">
              <button
                type="button"
                className="pCancelBtn"
                onClick={() => setIsFaqPasteOpen(false)}
              >
                Hủy
              </button>
              <button
                type="button"
                className="pSubmitBtn"
                onClick={handleFaqQuickPaste}
              >
                Phân tích & Thêm
              </button>
            </div>
      ...
      {isVariantPasteOpen && (
        <div className="pModalOverlay">
          <div className="pModalContent small">
            ...
            <div className="pModalBody">
              <p className="pHint">
                Định dạng: <b>Tên | Màu | Cấu hình | Giá | Kho</b>
              </p>
              <textarea
                className="pQuickPasteArea"
                rows={10}
            ...
            <div className="pModalFooter">
              <button
                className="pCancelBtn"
                onClick={() => setIsVariantPasteOpen(false)}
              >
                Hủy
              </button>
              <button className="pSubmitBtn" onClick={handleVariantQuickPaste}>
                Phân tích & Thêm
              </button>
            </div>
          </div>
