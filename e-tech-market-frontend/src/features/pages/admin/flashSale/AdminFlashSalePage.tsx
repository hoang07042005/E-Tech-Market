...
export default function AdminFlashSalePage() {
  ...
  // Bulk state
  const [activeItemTab, setActiveItemTab] = useState<"single" | "bulk">(
    "single",
  );
  ...
  const selectedProduct = availableProducts.find(
    (p) => String(p.id) === addItemData.product_id,
  );
    ...
    try {
      await apiFetch(`/api/admin/flash-sales/${currentSale.id}/items`, {
        ...
        body: JSON.stringify({
          ...
          variant_id: addItemData.variant_id
            ? Number(addItemData.variant_id)
            : null,
          flash_sale_price: Number(addItemData.flash_sale_price),
          quantity_limit: addItemData.quantity_limit
            ? Number(addItemData.quantity_limit)
            : null,
        }),
      });
  ...
  const handleBulkDiscount = async (e: React.FormEvent) => {
    ...
    if (
      !confirm(
        `Bạn có chắc chắn muốn giảm giá đồng loạt ${percent}% cho TẤT CẢ sản phẩm trong cửa hàng cho chiến dịch này không?`,
      )
    ) {
      return;
    }
    ...
    try {
      const res = await apiFetch<{ message: string }>(
        `/api/admin/flash-sales/${currentSale.id}/bulk-discount`,
        {
          method: "POST",
          body: JSON.stringify({
            discount_percentage: percent,
            quantity_limit: bulkQuantityLimit
              ? Number(bulkQuantityLimit)
              : null,
          }),
        },
      );
  ...
  const handleRemoveItem = async (itemId: number) => {
    ...
    try {
      await apiFetch(
        `/api/admin/flash-sales/${currentSale.id}/items/${itemId}`,
        {
          method: "DELETE",
        },
      );
  ...
  const handleDeleteSale = async (saleId: number) => {
    ...
    if (
      !confirm(
        "Bạn có chắc chắn muốn XÓA vĩnh viễn chiến dịch Flash Sale này không?\n\n⚠️ Lưu ý: Tất cả sản phẩm trong chiến dịch cũng sẽ bị xóa theo.",
      )
    )
      return;
    try {
  ...
  const filteredSales = sales.filter((sale) => {
    ...
    const matchesStatus =
      statusFilter === "all" || statusObj.type === statusFilter;
    return matchesSearch && matchesStatus;
  });

  // --- ĐẾM SỐ LƯỢNG CHO METRICS ---
  const activeCount = sales.filter(
    (s) => getStatus(s).type === "active",
  ).length;
  const waitingCount = sales.filter(
    (s) => getStatus(s).type === "waiting",
  ).length;
  const totalProductsInActive = sales.reduce(
    (acc, s) => acc + (s.items_count || 0),
    0,
  );
    ...
    <div className="fs-dashboard-container">
      ...
      <div className="fs-main-header">
        <div className="fs-title-area">
          ...
          <p className="fs-page-subtitle">
            Thiết lập cấu hình thời gian và sản phẩm khuyến mãi chớp nhoáng theo
            dòng sự kiện.
          </p>
        </div>
        <button
      ...
      <div className="fs-filter-wrapper">
        ...
        <div className="fs-filter-tabs">
          {(["all", "active", "waiting", "ended", "paused"] as const).map(
            (tab) => {
            ...
            {filteredSales.length === 0 ? (
              <tr>
                <td colSpan={7} className="fs-empty-table">
                  📭 Không tìm thấy chương trình Flash Sale nào phù hợp tiêu chí
                  lọc.
                </td>
              </tr>
            ) : (
              filteredSales.map((s) => {
                ...
                return (
                  <tr key={s.id} className="fs-row-interactive">
                    <td>
                      <span className="fs-txt-id">#{s.id}</span>
                    </td>
                    <td>
                      <strong className="fs-txt-name">{s.name}</strong>
                    </td>
                    <td>{formatDateTime(s.start_at)}</td>
                    <td>{formatDateTime(s.end_at)}</td>
                    <td>
                      <span className="fs-badge-count">
                        📦 {s.items_count || 0} sản phẩm
                      </span>
                    </td>
                    <td>
                      <span
                    ...
                    <td>
                      <div className="fs-action-group">
                        <button
                          className="fs-action-btn b-items"
                          onClick={() => handleOpenItems(s)}
                          title="Quản lý sản phẩm khuyến mãi"
                        >
                          Sản phẩm
                        </button>
                        <button
                          className="fs-icon-btn b-edit"
                          onClick={() => {
                            setEditingSale(s);
                        ...
                        <button
                          className="fs-icon-btn b-delete"
          ...
          <div className="fs-modal-box max-w-form animate-fade-in">
              ...
              <div>
                <h3 className="fs-modal-title">
                  {editingSale
                    ...
                    : "🚀 Cấu Hình Chiến Dịch Mới"}
                </h3>
                <p className="fs-modal-subtitle">
                  Thiết lập thông tin tên gọi cốt lõi và thời gian tự động
                  mở/đóng.
                </p>
              </div>
              <button
            ...
            <form onSubmit={handleSubmitSale} className="fs-form-body">
              ...
              <div className="fs-form-group bg-light-panel">
                ...
                >
                  <option value="active">
                    Hoạt động tự động (Đẩy ra client theo giờ hẹn)
                  </option>
                  <option value="waiting">Chờ phê duyệt / Lưu tạm thời</option>
                  <option value="paused">
                    Khóa tạm ngưng (Ẩn hoàn toàn khỏi hệ thống)
                  </option>
                  <option value="ended">Đã kết thúc chiến dịch sớm</option>
                </select>
                <p className="fs-input-hint">
                  * Hệ thống sẽ kiểm tra điều kiện thời gian thực để chuyển đổi
                  trạng thái hiển thị tự động khi bạn cài đặt ở chế độ "Hoạt
                  động tự động".
                </p>
              </div>
          ...
          <div className="fs-modal-box max-w-items animate-fade-in">
            ...
            <div className="fs-modal-header border-b">
              <div>
                <div className="fs-modal-title-with-icon">
                  ...
                  <h3 className="fs-modal-title">
                    Phân bổ sản phẩm tham gia khuyến mãi
                  </h3>
                </div>
                <p className="fs-modal-subtitle">
                  Đang thao tác trên chiến dịch:{" "}
                  <strong className="txt-highlight">{currentSale.name}</strong>
                </p>
              </div>
              <button
                className="fs-modal-close"
            ...
            <div className="fs-modal-split-layout">
                ...
                {activeItemTab === "single" ? (
                  <div className="fs-tab-content-panel side-form">
                    ...
                    <div className="fs-form-group">
                      ...
                      <select
                        ...
                        onChange={(e) => {
                          ...
                          setAddItemData({
                            ...addItemData,
                            variant_id: vId,
                            flash_sale_price: variant
                              ? String(variant.price)
                              : addItemData.flash_sale_price,
                          });
                        }}
                        disabled={
                      ...
                      >
                        <option value="">
                          -- Toàn bộ phiên bản mặc định --
                        </option>
                        {selectedProduct?.variants?.map((v) => (
                          <option key={v.id} value={v.id}>
                            {v.variant_name} ({Number(v.price).toLocaleString()}
                            đ)
                          </option>
                        ))}
                      </select>
                    ...
                    <button
                      ...
                      type="button"
                      className="fs-btn-primary w-100 h-input mt-sm"
                    >
                      Thêm vào danh sách →
                    </button>
                  </div>
                  ...
                  <form
                    onSubmit={handleBulkDiscount}
                    className="fs-tab-content-panel bg-orange-tint side-form"
                  >
                    <div className="fs-info-notice-bar">
                      💡 Tự động cấu hình giảm giá nhanh cho toàn sàn dựa theo %
                      dựa trên giá gốc gốc.
                    </div>

                    <div className="fs-form-group">
                      <label className="fs-form-label">
                        1. Mức chiết khấu giảm giá (%)
                      </label>
                      <div className="fs-input-prefix-wrapper">
                        <input
                          type="number"
                          ...
                          onChange={(e) =>
                            setBulkDiscountPercentage(e.target.value)
                          }
                          required
                        />
                        <span className="fs-input-unit-tag">%</span>
                    ...
                    <button
                      ...
                      className="fs-btn-gradient w-100 h-input mt-sm"
                    >
                      {isBulkApplying
              ...
              <div className="fs-modal-right-column">
                <h4 className="fs-section-small-title">
                  Danh sách sản phẩm được phân bổ hiện tại
                </h4>
                      ...
                      {!currentSale.items || currentSale.items.length === 0 ? (
                        <tr>
                          <td colSpan={5} className="fs-empty-table-sub">
                            ...
                            Chưa có sản phẩm nào được thiết lập giá ưu đãi cho
                            khung giờ này.
                          </td>
                        </tr>
                      ) : (
                        currentSale.items.map((item) => (
                          <tr key={item.id} className="fs-sub-row">
                            <td>
                              <div className="fs-product-meta-block">
                                ...
                                <div className="fs-product-info">
                                  <div className="fs-p-name">
                                    {item.product?.name ||
                                      "Sản phẩm đã gỡ khỏi kho"}
                                  </div>
                                  {item.variant ? (
                                    <div className="fs-p-variant">
                                      {item.variant.variant_name}
                                    </div>
                                  ) : (
                                    <div className="fs-p-all-variant">
                                      Mặc định
                                    </div>
                                  )}
                                </div>
                            ...
                            <td>
                              <span className="fs-p-sale-price">
                                {Number(item.flash_sale_price).toLocaleString()}
                                đ
                              </span>
                            </td>
                            <td>
                              <span
                                className={`fs-p-limit-badge ${item.quantity_limit ? "" : "infinite"}`}
                              >
                            ...
                            <td>
                              <div className="fs-progress-block">
                                ...
                                <span className="fs-progress-text">
                                  Đã bán: <strong>{item.sold_quantity}</strong>
                                </span>
                              </div>
                            </td>
                            <td style={{ textAlign: "center" }}>
                              <button
                                className="fs-p-btn-remove"
