...
export function clearAuthToken(): void {
  ...
  console.log(
    "[clearAuthToken] Called, current value:",
    current ? current.slice(0, 20) + "..." : "null",
  );
  try {
...
export function notifyGlobalError(message: string) {
  if (
    ...
    window.dispatchEvent(
      new CustomEvent("global-error", { detail: { message } }),
    );
  }
}
...
export async function apiFetch<T>(
  ...
  const { silent401 = false, ...fetchOptions } = options as RequestInit & {
    silent401?: boolean;
  };
  ...
  if (!API_BASE_URL) {
    ...
    effectivePath = normalizedPath.startsWith("/api")
      ? normalizedPath
      : `/api${normalizedPath}`;
  } else {
  ...
  if (token) {
    ...
    console.log(
      "[apiFetch] Token found, adding Bearer header",
      token.slice(0, 20) + "...",
    );
  } else {
